...
- **Consent gate (never inferred)** — `assertWritebackAuthorized` refuses a plan unless it carries `approved: true` **and** an `approvalId`, plus an idempotency key. This enforces the spec rule *"never infer missing repair consent."*
