---
document_id: FD-90186256
title: "Job and Workflow Architecture"
...
# Job and Workflow Architecture

## Purpose

Defines durable asynchronous execution without requiring a paid queue or workflow API.

## Scope

This document is the implementation authority for Job and Workflow Architecture. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Job and Workflow Architecture.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Use pg-boss queues partitioned by workload class, not by tenant.
- Every job payload contains schema version, job ID, tenant ID, actor or system principal, correlation ID, operation ID, and resource references—not raw secrets.
- Use unique keys for singleton or idempotent work and checkpoints for large catalogs.
- Retry only classified transient failures with bounded exponential backoff and jitter.
- Move exhausted jobs to a dead-letter state; never discard silently.
- Provide cancellation, pause, resume, and tenant quota checks at batch boundaries.
- Use an outbox transaction for events triggered by database mutations.
