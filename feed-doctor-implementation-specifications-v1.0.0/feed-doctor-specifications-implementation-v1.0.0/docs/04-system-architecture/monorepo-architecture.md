...
Defines package boundaries, dependency direction, build graph, ownership, and prohibited imports.

## Scope

This document is the implementation authority for Monorepo Architecture. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Monorepo Architecture.
...
- Packages: `domain`, `contracts`, `database`, `auth`, `connectors`, `diagnostics`, `repairs`, `jobs`, `analytics`, `seo`, `ui`, `config`, `observability`, `testing`.
...
- Dependency cycles are release-blocking.
