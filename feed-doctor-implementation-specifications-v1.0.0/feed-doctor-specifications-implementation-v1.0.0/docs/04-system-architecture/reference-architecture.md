...
Defines the production system decomposition, runtime topology, trust boundaries, data flows, deployment portability, and scaling path for Feed Doctor.

## Scope

This document is the implementation authority for Reference Architecture. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Reference Architecture.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The repository MUST be a TypeScript monorepo with deployable web, worker, and scheduled-maintenance applications plus shared domain, database, connector, contract, UI, and testing packages.
- The Next.js application MUST serve public SEO pages, authenticated application routes, and bounded request-response APIs; it MUST NOT execute unbounded catalog work inside request lifetimes.
- Durable work MUST execute through pg-boss jobs stored in PostgreSQL with explicit queue names, tenant fairness, retries, dead-letter handling, cancellation, and checkpoints.
- PostgreSQL is the transactional source of truth for identity, tenancy, connector metadata, catalogs, issues, repairs, jobs, analytics summaries, and content metadata.
- Object storage contains encrypted or access-controlled imports, exports, diagnostic artifacts, and large snapshots that do not belong in row storage.
- Every external platform is isolated behind a versioned connector adapter and normalized domain contract.
- Public search pages use server rendering or static generation with controlled revalidation; authenticated routes are non-indexable.
- Scale-out MUST be achieved by adding stateless web instances and worker instances before changing domain contracts.
...
1. **Web application:** routing, server-rendered UI, authentication callbacks, lightweight APIs, public content.
2. **Worker application:** imports, synchronization, crawling, diagnostics, repair execution, exports, notifications, sitemap generation.
3. **Maintenance scheduler:** enqueues recurring work; it does not perform business work itself.
4. **PostgreSQL:** transactional data, job queue, first-party analytics, initial search.
5. **Object storage:** upload and generated artifact blobs.
6. **External adapters:** Shopify, WooCommerce, Google Merchant Center, SMTP, optional Stripe.
7. **Browser:** untrusted client; never receives provider secrets or unrestricted tenant identifiers.

## Trust Boundaries

Internet clients, external webhook senders, commerce platforms, object storage, email infrastructure, and optional billing providers are separate trust boundaries. Every crossing requires authentication, validation, rate limiting, correlation, replay protection where applicable, and redaction.

## Authoritative Data Flow

Source connector → immutable source snapshot → normalized catalog → deterministic validator execution → issue instances and evidence → prioritized repair options → previewed change set → approval → connector writeback → verification sync → resolution or rollback. No stage overwrites evidence from an earlier stage.
