...
Defines how the implementation remains usable at minimal cost while preserving a credible scale path.

## Scope

This document is the implementation authority for Cost-Aware Architecture. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Cost-Aware Architecture.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Prefer one PostgreSQL dependency in bootstrap: relational storage, pg-boss jobs, first-party analytics, and full-text search.
- Do not deploy Redis, Kafka, Elasticsearch, a warehouse, or a paid workflow engine until measured thresholds and an ADR justify them.
- Use streaming parsers and bounded batches to avoid memory-driven compute costs.
- Cache only when measurements justify it; correctness must not depend on cache presence.
- Use static or incrementally rendered public pages where freshness permits.
- Make crawler depth, scan frequency, retention, export size, and concurrency entitlement-controlled.
- Emit per-tenant resource usage and cost proxies before scale.
