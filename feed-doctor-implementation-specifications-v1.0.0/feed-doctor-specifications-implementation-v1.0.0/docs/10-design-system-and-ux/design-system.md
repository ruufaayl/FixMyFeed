---
document_id: FD-6AD84A3C
title: "Feed Doctor Design System"
...
# Feed Doctor Design System

## Purpose

Defines the accessible visual and interaction system for public, application, administrative, and embedded surfaces.

## Scope

This document is the implementation authority for Feed Doctor Design System. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Feed Doctor Design System.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Use design tokens for color, typography, spacing, radii, shadows, z-index, motion, breakpoints, and density.
- Default application layout supports data-dense desktop workflows and complete mobile access without hiding required actions.
- Severity, status, confidence, and risk use text, icon, and color redundancy.
- Forms validate on blur and submit; destructive validation is never only inline after loss of data.
- Tables support keyboard navigation, visible selection state, column labels, sorting semantics, pagination, and accessible bulk actions.
- Motion respects `prefers-reduced-motion` and never blocks task completion.
- Copy uses merchant-language explanations first and exact provider terminology second.
...
Typography uses system font stacks to avoid font-hosting cost and privacy leakage. Base spacing is 4 px. Minimum target size is 44×44 CSS pixels for touch. Text contrast is at least 4.5:1, large text 3:1, and interactive boundaries 3:1 where applicable.

## Responsive Model

Desktop ≥1280: persistent navigation and multi-column evidence. Tablet 768–1279: collapsible navigation, stacked secondary panels. Mobile <768: single-column workflow, bottom-safe actions, horizontal table alternatives or card views; no critical data is removed.
