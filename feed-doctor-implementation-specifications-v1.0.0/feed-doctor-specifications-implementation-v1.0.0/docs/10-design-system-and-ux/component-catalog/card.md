---
document_id: FD-0BAB0F5D
title: "Component Specification — Card"
...
Defines the reusable design-system contract for the card component.

## Scope

This document is the implementation authority for Component Specification — Card. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Component Specification — Card.
...
The `card` component exposes only domain-neutral props: identifier, label, description, value/state, disabled/busy/invalid flags, size/density, permitted actions, and event callbacks. Provider payloads and tenant authorization logic are resolved before invocation.
