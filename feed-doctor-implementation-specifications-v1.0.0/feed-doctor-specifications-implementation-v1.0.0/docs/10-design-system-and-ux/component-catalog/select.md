---
document_id: FD-8493DB16
title: "Component Specification — Select"
...
Defines the reusable design-system contract for the select component.

## Scope

This document is the implementation authority for Component Specification — Select. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Component Specification — Select.
...
The `select` component exposes only domain-neutral props: identifier, label, description, value/state, disabled/busy/invalid flags, size/density, permitted actions, and event callbacks. Provider payloads and tenant authorization logic are resolved before invocation.
