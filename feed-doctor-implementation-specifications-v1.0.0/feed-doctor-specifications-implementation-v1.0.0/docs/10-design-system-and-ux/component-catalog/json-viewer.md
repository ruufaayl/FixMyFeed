---
document_id: FD-3AC58A78
title: "Component Specification — Json Viewer"
...
# Component Specification — Json Viewer

## Purpose

Defines the reusable design-system contract for the json viewer component.

## Scope

This document is the implementation authority for Component Specification — Json Viewer. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Component Specification — Json Viewer.
...
The `json_viewer` component exposes only domain-neutral props: identifier, label, description, value/state, disabled/busy/invalid flags, size/density, permitted actions, and event callbacks. Provider payloads and tenant authorization logic are resolved before invocation.
