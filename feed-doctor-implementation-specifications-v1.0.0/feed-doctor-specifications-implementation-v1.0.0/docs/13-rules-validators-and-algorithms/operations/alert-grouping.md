---
document_id: FD-5F02A038
title: "Algorithm Specification — Alert Grouping"
...
# Algorithm Specification — Alert Grouping

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for alert grouping.

## Scope

This document is the implementation authority for Algorithm Specification — Alert Grouping. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `alert-grouping` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-ALERT-GROUPING`
