---
document_id: FD-5D2ECE6C
title: "Algorithm Specification — Stale Connection Detection"
...
# Algorithm Specification — Stale Connection Detection

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for stale connection detection.

## Scope

This document is the implementation authority for Algorithm Specification — Stale Connection Detection. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `stale-connection-detection` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-STALE-CONNECTION-DETECTION`
