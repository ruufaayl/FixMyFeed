---
document_id: FD-B3CB95B1
title: "Algorithm Specification — Retry Backoff"
...
# Algorithm Specification — Retry Backoff

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for retry backoff.

## Scope

This document is the implementation authority for Algorithm Specification — Retry Backoff. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `retry-backoff` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-RETRY-BACKOFF`
