---
document_id: FD-89BA87D7
title: "Algorithm Specification — Rule Action Execution"
...
# Algorithm Specification — Rule Action Execution

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for rule action execution.

## Scope

This document is the implementation authority for Algorithm Specification — Rule Action Execution. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `rule-action-execution` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-RULE-ACTION-EXECUTION`
