---
document_id: FD-3E5DA651
title: "Algorithm Specification — Rule Ordering"
...
# Algorithm Specification — Rule Ordering

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for rule ordering.

## Scope

This document is the implementation authority for Algorithm Specification — Rule Ordering. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `rule-ordering` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-RULE-ORDERING`
