---
document_id: FD-C6C25596
title: "Algorithm Specification — Rule Scope Resolution"
...
# Algorithm Specification — Rule Scope Resolution

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for rule scope resolution.

## Scope

This document is the implementation authority for Algorithm Specification — Rule Scope Resolution. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `rule-scope-resolution` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-RULE-SCOPE-RESOLUTION`
