---
document_id: FD-F97AE69E
title: "Algorithm Specification — Rule Condition Evaluation"
...
# Algorithm Specification — Rule Condition Evaluation

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for rule condition evaluation.

## Scope

This document is the implementation authority for Algorithm Specification — Rule Condition Evaluation. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `rule-condition-evaluation` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-RULE-CONDITION-EVALUATION`
