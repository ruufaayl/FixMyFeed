---
document_id: FD-D59AD2CC
title: "Algorithm Specification — Rule Precedence"
...
# Algorithm Specification — Rule Precedence

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for rule precedence.

## Scope

This document is the implementation authority for Algorithm Specification — Rule Precedence. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `rule-precedence` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-RULE-PRECEDENCE`
