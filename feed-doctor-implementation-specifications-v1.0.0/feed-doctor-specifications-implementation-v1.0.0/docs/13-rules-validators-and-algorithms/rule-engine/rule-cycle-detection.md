---
document_id: FD-18AA0A79
title: "Algorithm Specification — Rule Cycle Detection"
...
# Algorithm Specification — Rule Cycle Detection

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for rule cycle detection.

## Scope

This document is the implementation authority for Algorithm Specification — Rule Cycle Detection. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `rule-cycle-detection` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-RULE-CYCLE-DETECTION`
