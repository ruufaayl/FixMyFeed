...
Defines how validators are registered, versioned, executed, evidenced, tested, and mapped to issue definitions without generative dependencies.

## Scope

This document is the implementation authority for Deterministic Validation Framework. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Deterministic Validation Framework.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Every validator has a stable ID, semantic version, scope, required fields, supported destinations and markets, severity mapping, evidence schema, and remediation references.
- Validators are pure with respect to normalized input and versioned reference data; network-dependent checks are isolated acquisition stages with captured evidence.
- A validator cannot directly write source or destination data.
- Unknown or missing input produces `not_evaluated` or a specific data-quality finding, never an invented pass.
- Execution is batchable, cancellable, checkpointed, and deterministic across retries.
- Changing detection semantics creates a new validator version and supports baseline comparison before rollout.
...
Each result contains validator ID/version, entity reference, status (`pass`, `fail`, `warning`, `not_applicable`, `not_evaluated`), issue definition ID, severity, confidence, evidence references, evaluated timestamp, reference-data version, and duration.

## Evaluation Order

Normalization → structural validation → identity validation → attribute validation → media and URL acquisition → cross-source consistency → policy-readiness checks → issue aggregation → impact and prioritization.
