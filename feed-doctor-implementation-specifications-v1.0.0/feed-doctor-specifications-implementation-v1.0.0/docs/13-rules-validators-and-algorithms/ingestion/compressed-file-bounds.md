---
document_id: FD-7A693FE9
title: "Algorithm Specification — Compressed File Bounds"
...
# Algorithm Specification — Compressed File Bounds

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for compressed file bounds.

## Scope

This document is the implementation authority for Algorithm Specification — Compressed File Bounds. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `compressed-file-bounds` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-COMPRESSED-FILE-BOUNDS`
