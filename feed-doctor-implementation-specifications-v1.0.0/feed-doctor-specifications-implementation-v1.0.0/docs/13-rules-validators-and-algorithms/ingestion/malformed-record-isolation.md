---
document_id: FD-6549DD49
title: "Algorithm Specification — Malformed Record Isolation"
...
# Algorithm Specification — Malformed Record Isolation

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for malformed record isolation.

## Scope

This document is the implementation authority for Algorithm Specification — Malformed Record Isolation. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `malformed-record-isolation` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-MALFORMED-RECORD-ISOLATION`
