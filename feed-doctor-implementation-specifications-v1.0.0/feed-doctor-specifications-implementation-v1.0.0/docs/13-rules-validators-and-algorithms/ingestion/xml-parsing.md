---
document_id: FD-B022179E
title: "Algorithm Specification — Xml Parsing"
...
# Algorithm Specification — Xml Parsing

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for xml parsing.

## Scope

This document is the implementation authority for Algorithm Specification — Xml Parsing. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `xml-parsing` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-XML-PARSING`
