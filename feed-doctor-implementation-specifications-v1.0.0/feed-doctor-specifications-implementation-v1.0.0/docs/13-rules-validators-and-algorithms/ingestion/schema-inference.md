---
document_id: FD-CDB71153
title: "Algorithm Specification — Schema Inference"
...
# Algorithm Specification — Schema Inference

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for schema inference.

## Scope

This document is the implementation authority for Algorithm Specification — Schema Inference. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `schema-inference` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-SCHEMA-INFERENCE`
