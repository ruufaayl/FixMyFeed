---
document_id: FD-F097F40C
title: "Algorithm Specification — Repair Rollback Generation"
...
# Algorithm Specification — Repair Rollback Generation

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for repair rollback generation.

## Scope

This document is the implementation authority for Algorithm Specification — Repair Rollback Generation. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `repair-rollback-generation` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-REPAIR-ROLLBACK-GENERATION`
