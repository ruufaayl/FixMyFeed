---
document_id: FD-7BEB85C0
title: "Algorithm Specification — Attribute Source Selection"
...
# Algorithm Specification — Attribute Source Selection

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for attribute source selection.

## Scope

This document is the implementation authority for Algorithm Specification — Attribute Source Selection. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `attribute-source-selection` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-ATTRIBUTE-SOURCE-SELECTION`
