---
document_id: FD-6FA05DB6
title: "Algorithm Specification — Availability Repair"
...
# Algorithm Specification — Availability Repair

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for availability repair.

## Scope

This document is the implementation authority for Algorithm Specification — Availability Repair. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `availability-repair` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-AVAILABILITY-REPAIR`
