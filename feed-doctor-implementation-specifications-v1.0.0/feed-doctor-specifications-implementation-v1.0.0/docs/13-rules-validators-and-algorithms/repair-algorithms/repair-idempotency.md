---
document_id: FD-9A7CDD66
title: "Algorithm Specification — Repair Idempotency"
...
# Algorithm Specification — Repair Idempotency

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for repair idempotency.

## Scope

This document is the implementation authority for Algorithm Specification — Repair Idempotency. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `repair-idempotency` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-REPAIR-IDEMPOTENCY`
