---
document_id: FD-2D393215
title: "Algorithm Specification — Repair Conflict Detection"
...
# Algorithm Specification — Repair Conflict Detection

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for repair conflict detection.

## Scope

This document is the implementation authority for Algorithm Specification — Repair Conflict Detection. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `repair-conflict-detection` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-REPAIR-CONFLICT-DETECTION`
