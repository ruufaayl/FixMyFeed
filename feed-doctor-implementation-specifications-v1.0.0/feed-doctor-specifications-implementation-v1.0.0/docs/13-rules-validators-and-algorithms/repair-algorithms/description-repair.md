---
document_id: FD-F7C1160C
title: "Algorithm Specification — Description Repair"
...
# Algorithm Specification — Description Repair

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for description repair.

## Scope

This document is the implementation authority for Algorithm Specification — Description Repair. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `description-repair` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-DESCRIPTION-REPAIR`
