---
document_id: FD-39FE3523
title: "Algorithm Specification — Remediation Option Selection"
...
# Algorithm Specification — Remediation Option Selection

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for remediation option selection.

## Scope

This document is the implementation authority for Algorithm Specification — Remediation Option Selection. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `remediation-option-selection` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-REMEDIATION-OPTION-SELECTION`
**Determinism:** Required
**Authority:** `Repair plan only; execution requires approval`
**Side effects:** `Only through connector executor when explicitly specified`
