---
document_id: FD-EA6CB85F
title: "Algorithm Specification — Source To Destination Matching"
...
# Algorithm Specification — Source To Destination Matching

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for source to destination matching.

## Scope

This document is the implementation authority for Algorithm Specification — Source To Destination Matching. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `source-to-destination-matching` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-SOURCE-TO-DESTINATION-MATCHING`
