---
document_id: FD-7BED19B0
title: "Algorithm Specification — Issue Key Generation"
...
# Algorithm Specification — Issue Key Generation

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for issue key generation.

## Scope

This document is the implementation authority for Algorithm Specification — Issue Key Generation. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `issue-key-generation` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-ISSUE-KEY-GENERATION`
