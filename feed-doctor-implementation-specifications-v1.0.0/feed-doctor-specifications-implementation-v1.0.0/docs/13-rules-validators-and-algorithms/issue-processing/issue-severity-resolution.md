---
document_id: FD-F22D030D
title: "Algorithm Specification — Issue Severity Resolution"
...
# Algorithm Specification — Issue Severity Resolution

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for issue severity resolution.

## Scope

This document is the implementation authority for Algorithm Specification — Issue Severity Resolution. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `issue-severity-resolution` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-ISSUE-SEVERITY-RESOLUTION`
