---
document_id: FD-47DCF4B3
title: "Algorithm Specification — Root Cause Ranking"
...
# Algorithm Specification — Root Cause Ranking

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for root cause ranking.

## Scope

This document is the implementation authority for Algorithm Specification — Root Cause Ranking. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `root-cause-ranking` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-ROOT-CAUSE-RANKING`
