---
document_id: FD-1A5E0817
title: "Algorithm Specification — Issue Priority Scoring"
...
# Algorithm Specification — Issue Priority Scoring

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for issue priority scoring.

## Scope

This document is the implementation authority for Algorithm Specification — Issue Priority Scoring. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `issue-priority-scoring` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-ISSUE-PRIORITY-SCORING`
