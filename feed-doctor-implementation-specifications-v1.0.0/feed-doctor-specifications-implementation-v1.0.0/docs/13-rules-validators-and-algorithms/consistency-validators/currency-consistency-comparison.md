---
document_id: FD-DDDC296B
title: "Algorithm Specification — Currency Consistency Comparison"
...
# Algorithm Specification — Currency Consistency Comparison

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for currency consistency comparison.

## Scope

This document is the implementation authority for Algorithm Specification — Currency Consistency Comparison. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `currency-consistency-comparison` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-CURRENCY-CONSISTENCY-COMPARISON`
