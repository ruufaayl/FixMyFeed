---
document_id: FD-84FBB7D0
title: "Algorithm Specification — Variant Consistency Comparison"
...
# Algorithm Specification — Variant Consistency Comparison

## Purpose

Defines the deterministic algorithm, inputs, outputs, invariants, complexity, evidence, failure behavior, and evaluation tests for variant consistency comparison.

## Scope

This document is the implementation authority for Algorithm Specification — Variant Consistency Comparison. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- The `variant-consistency-comparison` algorithm MUST be deterministic for identical normalized inputs and reference-data versions.
...
**Stable identifier:** `ALG-VARIANT-CONSISTENCY-COMPARISON`
