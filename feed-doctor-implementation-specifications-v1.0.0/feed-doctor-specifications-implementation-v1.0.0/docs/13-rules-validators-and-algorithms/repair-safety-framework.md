---
document_id: FD-AB62C4DE
title: "Repair Safety Framework"
...
# Repair Safety Framework

## Purpose

Defines repair classes, prerequisites, preview, approval, execution, verification, and rollback.

## Scope

This document is the implementation authority for Repair Safety Framework. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Repair Safety Framework.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Class 0 explains only; Class 1 produces instructions; Class 2 proposes deterministic reversible edits; Class 3 performs approved external writes; Class 4 represents provider-controlled reviews or appeals and cannot be simulated.
- Every repair option declares authority source, fields touched, affected entities, dependencies, expected issue resolution, possible new issues, reversibility, and verification method.
- Automatic execution is disabled by default globally and per connection.
- Bulk plans partition by connector, safety class, market, source version, and rollback compatibility.
- A repair whose evidence or source version is stale becomes blocked before mutation.
- Verification uses a fresh connector read and diagnostic re-evaluation.
