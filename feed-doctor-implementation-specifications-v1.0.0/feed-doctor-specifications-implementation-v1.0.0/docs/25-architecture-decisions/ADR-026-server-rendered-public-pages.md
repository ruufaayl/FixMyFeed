---
document_id: FD-464698F7
title: "ADR-026: Server-Rendered Public Pages"
...
# ADR-026: Server-Rendered Public Pages

## Purpose

Records the binding architecture decision: Use server-rendered or statically generated public SEO pages with controlled revalidation.

## Scope

This document is the implementation authority for ADR-026: Server-Rendered Public Pages. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of ADR-026: Server-Rendered Public Pages.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Use server-rendered or statically generated public SEO pages with controlled revalidation.
- Implementations MUST comply until this ADR is superseded by a later approved ADR.
- Agents MUST update affected specifications, tests, migration plans, and cost/security analysis when proposing a superseding decision.
...
Feed Doctor must support production-grade commerce integrations, large catalogs, public SEO tooling, safe repairs, and a minimal-cost bootstrap. The decision must reduce ambiguity for autonomous agents and preserve a credible scale path.
...
Benefits, constraints, operational ownership, migration implications, failure modes, and provider portability are accepted as part of this decision. Any negative consequence must be handled in the related implementation specification rather than ignored.

## Alternatives Rejected

Alternatives that add mandatory paid services, couple domain logic to a vendor, weaken determinism, or create unbounded operational cost are rejected unless future measured evidence justifies a superseding ADR.

## Compliance Evidence

The implementation must provide dependency graph checks, configuration validation, contract tests, deployment evidence, and repository traceability demonstrating adherence.
