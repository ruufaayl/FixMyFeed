---
document_id: FD-5C73B379
title: "SEO and Public Content Architecture"
...
# SEO and Public Content Architecture

## Purpose

Defines the scalable, evidence-backed public resource platform, programmatic page controls, rendering, linking, freshness, sitemaps, structured data, and conversion into Feed Doctor workflows.

## Scope

This document is the implementation authority for SEO and Public Content Architecture. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of SEO and Public Content Architecture.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Every indexable page solves one search intent and one merchant workflow.
- Pages are generated from reviewed structured content records, not raw provider text or unreviewed model output.
- Public HTML is server rendered or statically generated with canonical metadata and crawlable internal links.
- Indexability is a controlled state: draft, reviewed, indexable, noindex, redirected, retired.
- Programmatic pages require unique diagnosis, evidence, platform-specific steps, validation tool or example, and related issue links.
- Thin, duplicate, stale, unsupported, or unverified pages remain noindex.
- Authenticated or user-specific results are never indexable.
- Content sources, reviewer, source version, and last verified date are stored and displayed where useful.
- The content platform works without a paid CMS, keyword API, or AI content API.
...
- `/issues/{issue-slug}`
- `/issues/{issue-slug}/shopify`
- `/issues/{issue-slug}/woocommerce`
- `/attributes/{attribute-slug}`
- `/tools/{tool-slug}`
- `/guides/{guide-slug}`
- `/integrations/{integration-slug}`
- `/glossary/{term-slug}`
- `/compare/{comparison-slug}` only after legal/editorial review.

## Quality Gate

A page may become indexable only when it has authoritative sources, unique purpose, complete workflow, tested metadata, canonical correctness, valid internal links, non-duplicative body, useful examples or tool capability, accessibility pass, and named review owner.

## Content-to-Product Handoff

Public issue and tool pages can start an anonymous scan with explicit consent. Results are private and noindex. Account creation may save a result, but diagnostic value cannot be artificially withheld solely to force signup.
