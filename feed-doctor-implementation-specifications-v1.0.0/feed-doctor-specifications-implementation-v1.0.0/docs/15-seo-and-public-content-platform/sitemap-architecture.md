...
Defines automatic XML sitemap generation, partitioning, freshness, validation, submission readiness, and failure recovery.

## Scope

This document is the implementation authority for Sitemap Architecture. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Sitemap Architecture.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Generate a sitemap index at `/sitemap.xml` referencing type-specific sitemaps.
- Partition sitemaps by page type and below protocol URL/file limits with conservative headroom.
- Include only canonical, 200-status, indexable, reviewed URLs.
- `lastmod` reflects a material content change, not every build or request.
- Generate atomically from the content database and retain the last known valid artifact.
- Validate URLs, host, canonical state, duplicates, encoding, and inaccessible routes before publication.
- Robots.txt references the sitemap index.
- Agents prepare Search Console verification instructions and evidence, but owner completes account ownership steps.
...
`sitemap-core.xml`, `sitemap-issues-N.xml`, `sitemap-attributes-N.xml`, `sitemap-tools.xml`, `sitemap-guides-N.xml`, `sitemap-integrations.xml`, and localized variants when enabled.

## Failure Behavior

If generation fails, continue serving the previous valid sitemap and alert. Never publish a partial or empty sitemap caused by a transient database or build failure.
