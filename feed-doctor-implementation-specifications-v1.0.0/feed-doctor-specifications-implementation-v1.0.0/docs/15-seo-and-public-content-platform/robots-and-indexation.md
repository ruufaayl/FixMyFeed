---
document_id: FD-C5A06261
title: "Robots and Indexation Controls"
...
# Robots and Indexation Controls

## Purpose

Defines robots.txt, meta robots, X-Robots-Tag, canonical handling, environment safety, and accidental-indexation prevention.

## Scope

This document is the implementation authority for Robots and Indexation Controls. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Robots and Indexation Controls.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Non-production environments deny crawling globally and require access protection.
- Authenticated application, callbacks, API routes, exports, tool results, search results, and administrative routes are noindex.
- Robots.txt never attempts to secure private data; authorization remains mandatory.
- Canonical tags point only to equivalent public 200-status URLs.
- Redirected and retired pages are removed from sitemaps and use mapped redirects or 410 where appropriate.
- Query-parameter variants default to canonical or noindex according to an explicit allowlist.
