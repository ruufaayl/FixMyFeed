...
Defines JSON-LD generation, eligibility, validation, and truthful use across public pages.

## Scope

This document is the implementation authority for Structured Data Architecture. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Structured Data Architecture.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Use Organization and WebSite on appropriate global pages, BreadcrumbList on hierarchical pages, SoftwareApplication only where claims are supported, FAQPage only for visible genuine FAQs, and Article where editorial authorship exists.
- Do not mark tool outputs, hidden content, testimonials, ratings, or unavailable features with misleading schema.
- Structured data is generated from the same source as visible content.
- CI validates JSON syntax, required fields, URL consistency, and page visibility.
- Schema changes are versioned and monitored for search-engine errors.
