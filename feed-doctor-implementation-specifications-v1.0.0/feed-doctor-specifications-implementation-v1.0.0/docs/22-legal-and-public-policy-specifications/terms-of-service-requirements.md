...
Defines product and engineering requirements for Commercial terms, acceptable use, account responsibility, external-platform dependency, repair authorization, limitation, termination, and dispute clauses requiring qualified counsel.

## Scope

This document is the implementation authority for Terms Of Service Requirements. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Terms Of Service Requirements.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Commercial terms, acceptable use, account responsibility, external-platform dependency, repair authorization, limitation, termination, and dispute clauses requiring qualified counsel.
- Final legal language requires qualified legal review before production publication.
- Product behavior, data flows, retention, and user controls must match published language.
- Agents may prepare implementation and placeholders but may not represent unreviewed text as legal advice.
- Version and effective date are displayed and previous versions retained.
