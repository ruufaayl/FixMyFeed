...
Defines in-app, SMTP email, webhook, and optional chat notifications with preferences, suppression, retries, and no mandatory paid provider.

## Scope

This document is the implementation authority for Notification Architecture. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Notification Architecture.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- In-app notifications are the authoritative delivery channel.
- Email uses standard SMTP and can use owner-selected free or low-cost infrastructure; application behavior does not depend on a specific vendor.
- Every notification has urgency, category, deduplication key, recipient scope, template version, and expiration.
- Digestible events are grouped; critical security or writeback failures are not delayed into a digest.
- Delivery failures do not roll back underlying domain events.
- Users can configure non-essential channels while security-required notices remain deliverable where legally permitted.
