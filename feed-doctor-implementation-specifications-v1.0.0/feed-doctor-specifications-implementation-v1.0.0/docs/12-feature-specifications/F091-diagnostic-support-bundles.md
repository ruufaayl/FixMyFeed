---
document_id: FD-14EA5A82
title: "F091 — Diagnostic Support Bundles"
...
# F091 — Diagnostic Support Bundles

## Purpose

Defines the end-to-end production requirements for the diagnostic support bundles feature.

## Scope

This document is the implementation authority for F091 — Diagnostic Support Bundles. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F091 — Diagnostic Support Bundles.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `diagnostic support bundles` workflow using versioned domain and API contracts.
...
- `diagnostic_support_bundles_viewed` when the workflow becomes usable.
- `diagnostic_support_bundles_started` after validated user intent.
- `diagnostic_support_bundles_completed` only after durable success.
- `diagnostic_support_bundles_failed` with canonical error category and retryability.
- `diagnostic_support_bundles_cancelled` or `abandoned` when distinguishable.
