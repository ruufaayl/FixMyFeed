---
document_id: FD-C1D022C2
title: "F084 — Public Api"
...
Defines the end-to-end production requirements for the public api feature.

## Scope

This document is the implementation authority for F084 — Public Api. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F084 — Public Api.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `public api` workflow using versioned domain and API contracts.
...
- `public_api_viewed` when the workflow becomes usable.
- `public_api_started` after validated user intent.
- `public_api_completed` only after durable success.
- `public_api_failed` with canonical error category and retryability.
- `public_api_cancelled` or `abandoned` when distinguishable.
