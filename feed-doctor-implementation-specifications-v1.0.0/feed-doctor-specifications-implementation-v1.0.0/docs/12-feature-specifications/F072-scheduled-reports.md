---
document_id: FD-07EAFAF8
title: "F072 — Scheduled Reports"
...
# F072 — Scheduled Reports

## Purpose

Defines the end-to-end production requirements for the scheduled reports feature.

## Scope

This document is the implementation authority for F072 — Scheduled Reports. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F072 — Scheduled Reports.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `scheduled reports` workflow using versioned domain and API contracts.
...
- `scheduled_reports_viewed` when the workflow becomes usable.
- `scheduled_reports_started` after validated user intent.
- `scheduled_reports_completed` only after durable success.
- `scheduled_reports_failed` with canonical error category and retryability.
- `scheduled_reports_cancelled` or `abandoned` when distinguishable.
