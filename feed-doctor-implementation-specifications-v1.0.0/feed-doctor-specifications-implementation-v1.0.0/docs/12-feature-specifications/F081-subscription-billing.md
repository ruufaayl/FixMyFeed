---
document_id: FD-0F00F673
title: "F081 — Subscription Billing"
...
# F081 — Subscription Billing

## Purpose

Defines the end-to-end production requirements for the subscription billing feature.

## Scope

This document is the implementation authority for F081 — Subscription Billing. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F081 — Subscription Billing.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `subscription billing` workflow using versioned domain and API contracts.
...
- `subscription_billing_viewed` when the workflow becomes usable.
- `subscription_billing_started` after validated user intent.
- `subscription_billing_completed` only after durable success.
- `subscription_billing_failed` with canonical error category and retryability.
- `subscription_billing_cancelled` or `abandoned` when distinguishable.
