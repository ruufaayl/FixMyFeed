---
document_id: FD-AC803C88
title: "F061 — Alert Grouping"
...
Defines the end-to-end production requirements for the alert grouping feature.

## Scope

This document is the implementation authority for F061 — Alert Grouping. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F061 — Alert Grouping.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `alert grouping` workflow using versioned domain and API contracts.
...
- `alert_grouping_viewed` when the workflow becomes usable.
- `alert_grouping_started` after validated user intent.
- `alert_grouping_completed` only after durable success.
- `alert_grouping_failed` with canonical error category and retryability.
- `alert_grouping_cancelled` or `abandoned` when distinguishable.
