---
document_id: FD-F6015CB6
title: "F071 — Report Builder"
...
Defines the end-to-end production requirements for the report builder feature.

## Scope

This document is the implementation authority for F071 — Report Builder. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F071 — Report Builder.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `report builder` workflow using versioned domain and API contracts.
...
- `report_builder_viewed` when the workflow becomes usable.
- `report_builder_started` after validated user intent.
- `report_builder_completed` only after durable success.
- `report_builder_failed` with canonical error category and retryability.
- `report_builder_cancelled` or `abandoned` when distinguishable.
