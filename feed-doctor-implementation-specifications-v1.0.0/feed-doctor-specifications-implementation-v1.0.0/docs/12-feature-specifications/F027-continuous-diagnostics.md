---
document_id: FD-E3434D73
title: "F027 — Continuous Diagnostics"
...
# F027 — Continuous Diagnostics

## Purpose

Defines the end-to-end production requirements for the continuous diagnostics feature.

## Scope

This document is the implementation authority for F027 — Continuous Diagnostics. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F027 — Continuous Diagnostics.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `continuous diagnostics` workflow using versioned domain and API contracts.
...
- `continuous_diagnostics_viewed` when the workflow becomes usable.
- `continuous_diagnostics_started` after validated user intent.
- `continuous_diagnostics_completed` only after durable success.
- `continuous_diagnostics_failed` with canonical error category and retryability.
- `continuous_diagnostics_cancelled` or `abandoned` when distinguishable.
