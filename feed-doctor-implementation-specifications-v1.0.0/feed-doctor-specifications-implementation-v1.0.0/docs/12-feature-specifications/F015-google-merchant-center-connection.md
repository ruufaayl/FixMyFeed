---
document_id: FD-A1B45AA5
title: "F015 — Google Merchant Center Connection"
...
# F015 — Google Merchant Center Connection

## Purpose

Defines the end-to-end production requirements for the google merchant center connection feature.

## Scope

This document is the implementation authority for F015 — Google Merchant Center Connection. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F015 — Google Merchant Center Connection.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `google merchant center connection` workflow using versioned domain and API contracts.
...
- `google_merchant_center_connection_viewed` when the workflow becomes usable.
- `google_merchant_center_connection_started` after validated user intent.
- `google_merchant_center_connection_completed` only after durable success.
- `google_merchant_center_connection_failed` with canonical error category and retryability.
- `google_merchant_center_connection_cancelled` or `abandoned` when distinguishable.
