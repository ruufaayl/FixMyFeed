---
document_id: FD-FF6A026B
title: "F097 — Public Title Checker"
...
# F097 — Public Title Checker

## Purpose

Defines the end-to-end production requirements for the public title checker feature.

## Scope

This document is the implementation authority for F097 — Public Title Checker. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F097 — Public Title Checker.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `public title checker` workflow using versioned domain and API contracts.
...
- `public_title_checker_viewed` when the workflow becomes usable.
- `public_title_checker_started` after validated user intent.
- `public_title_checker_completed` only after durable success.
- `public_title_checker_failed` with canonical error category and retryability.
- `public_title_checker_cancelled` or `abandoned` when distinguishable.
