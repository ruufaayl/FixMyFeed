---
document_id: FD-7925F40D
title: "F086 — Data Export"
...
Defines the end-to-end production requirements for the data export feature.

## Scope

This document is the implementation authority for F086 — Data Export. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F086 — Data Export.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `data export` workflow using versioned domain and API contracts.
...
- `data_export_viewed` when the workflow becomes usable.
- `data_export_started` after validated user intent.
- `data_export_completed` only after durable success.
- `data_export_failed` with canonical error category and retryability.
- `data_export_cancelled` or `abandoned` when distinguishable.
