---
document_id: FD-B7F5157F
title: "F020 — Full Reconciliation"
...
# F020 — Full Reconciliation

## Purpose

Defines the end-to-end production requirements for the full reconciliation feature.

## Scope

This document is the implementation authority for F020 — Full Reconciliation. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F020 — Full Reconciliation.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `full reconciliation` workflow using versioned domain and API contracts.
...
- `full_reconciliation_viewed` when the workflow becomes usable.
- `full_reconciliation_started` after validated user intent.
- `full_reconciliation_completed` only after durable success.
- `full_reconciliation_failed` with canonical error category and retryability.
- `full_reconciliation_cancelled` or `abandoned` when distinguishable.
