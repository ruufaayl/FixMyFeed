---
document_id: FD-C2E9A0E4
title: "F033 — Issue Evidence Generation"
...
# F033 — Issue Evidence Generation

## Purpose

Defines the end-to-end production requirements for the issue evidence generation feature.

## Scope

This document is the implementation authority for F033 — Issue Evidence Generation. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F033 — Issue Evidence Generation.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `issue evidence generation` workflow using versioned domain and API contracts.
...
- `issue_evidence_generation_viewed` when the workflow becomes usable.
- `issue_evidence_generation_started` after validated user intent.
- `issue_evidence_generation_completed` only after durable success.
- `issue_evidence_generation_failed` with canonical error category and retryability.
- `issue_evidence_generation_cancelled` or `abandoned` when distinguishable.
