---
document_id: FD-256EE106
title: "F100 — Public Availability Checker"
...
# F100 — Public Availability Checker

## Purpose

Defines the end-to-end production requirements for the public availability checker feature.

## Scope

This document is the implementation authority for F100 — Public Availability Checker. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F100 — Public Availability Checker.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `public availability checker` workflow using versioned domain and API contracts.
...
- `public_availability_checker_viewed` when the workflow becomes usable.
- `public_availability_checker_started` after validated user intent.
- `public_availability_checker_completed` only after durable success.
- `public_availability_checker_failed` with canonical error category and retryability.
- `public_availability_checker_cancelled` or `abandoned` when distinguishable.
