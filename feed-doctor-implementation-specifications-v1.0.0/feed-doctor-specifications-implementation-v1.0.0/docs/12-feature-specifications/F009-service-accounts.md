---
document_id: FD-B9A27025
title: "F009 — Service Accounts"
...
# F009 — Service Accounts

## Purpose

Defines the end-to-end production requirements for the service accounts feature.

## Scope

This document is the implementation authority for F009 — Service Accounts. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F009 — Service Accounts.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `service accounts` workflow using versioned domain and API contracts.
...
- `service_accounts_viewed` when the workflow becomes usable.
- `service_accounts_started` after validated user intent.
- `service_accounts_completed` only after durable success.
- `service_accounts_failed` with canonical error category and retryability.
- `service_accounts_cancelled` or `abandoned` when distinguishable.
