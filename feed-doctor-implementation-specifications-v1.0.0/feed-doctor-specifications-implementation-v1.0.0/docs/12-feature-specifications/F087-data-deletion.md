---
document_id: FD-DA9A9585
title: "F087 — Data Deletion"
...
Defines the end-to-end production requirements for the data deletion feature.

## Scope

This document is the implementation authority for F087 — Data Deletion. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F087 — Data Deletion.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `data deletion` workflow using versioned domain and API contracts.
...
- `data_deletion_viewed` when the workflow becomes usable.
- `data_deletion_started` after validated user intent.
- `data_deletion_completed` only after durable success.
- `data_deletion_failed` with canonical error category and retryability.
- `data_deletion_cancelled` or `abandoned` when distinguishable.
