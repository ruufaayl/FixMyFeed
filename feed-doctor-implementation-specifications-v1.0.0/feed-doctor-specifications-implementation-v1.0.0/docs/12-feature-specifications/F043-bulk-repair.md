---
document_id: FD-6C7831A4
title: "F043 — Bulk Repair"
...
Defines the end-to-end production requirements for the bulk repair feature.

## Scope

This document is the implementation authority for F043 — Bulk Repair. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F043 — Bulk Repair.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `bulk repair` workflow using versioned domain and API contracts.
...
- `bulk_repair_viewed` when the workflow becomes usable.
- `bulk_repair_started` after validated user intent.
- `bulk_repair_completed` only after durable success.
- `bulk_repair_failed` with canonical error category and retryability.
- `bulk_repair_cancelled` or `abandoned` when distinguishable.
