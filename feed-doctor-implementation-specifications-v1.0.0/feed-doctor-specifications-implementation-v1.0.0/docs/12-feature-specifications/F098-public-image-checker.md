---
document_id: FD-F6711F7B
title: "F098 — Public Image Checker"
...
# F098 — Public Image Checker

## Purpose

Defines the end-to-end production requirements for the public image checker feature.

## Scope

This document is the implementation authority for F098 — Public Image Checker. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F098 — Public Image Checker.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `public image checker` workflow using versioned domain and API contracts.
...
- `public_image_checker_viewed` when the workflow becomes usable.
- `public_image_checker_started` after validated user intent.
- `public_image_checker_completed` only after durable success.
- `public_image_checker_failed` with canonical error category and retryability.
- `public_image_checker_cancelled` or `abandoned` when distinguishable.
