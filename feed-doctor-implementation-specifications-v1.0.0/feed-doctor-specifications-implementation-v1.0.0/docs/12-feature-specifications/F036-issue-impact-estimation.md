---
document_id: FD-88AF8C4D
title: "F036 — Issue Impact Estimation"
...
# F036 — Issue Impact Estimation

## Purpose

Defines the end-to-end production requirements for the issue impact estimation feature.

## Scope

This document is the implementation authority for F036 — Issue Impact Estimation. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F036 — Issue Impact Estimation.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `issue impact estimation` workflow using versioned domain and API contracts.
...
- `issue_impact_estimation_viewed` when the workflow becomes usable.
- `issue_impact_estimation_started` after validated user intent.
- `issue_impact_estimation_completed` only after durable success.
- `issue_impact_estimation_failed` with canonical error category and retryability.
- `issue_impact_estimation_cancelled` or `abandoned` when distinguishable.
