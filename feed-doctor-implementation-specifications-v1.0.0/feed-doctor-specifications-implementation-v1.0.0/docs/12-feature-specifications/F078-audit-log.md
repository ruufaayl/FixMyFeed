---
document_id: FD-3BAA73AD
title: "F078 — Audit Log"
...
Defines the end-to-end production requirements for the audit log feature.

## Scope

This document is the implementation authority for F078 — Audit Log. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F078 — Audit Log.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `audit log` workflow using versioned domain and API contracts.
...
- `audit_log_viewed` when the workflow becomes usable.
- `audit_log_started` after validated user intent.
- `audit_log_completed` only after durable success.
- `audit_log_failed` with canonical error category and retryability.
- `audit_log_cancelled` or `abandoned` when distinguishable.
