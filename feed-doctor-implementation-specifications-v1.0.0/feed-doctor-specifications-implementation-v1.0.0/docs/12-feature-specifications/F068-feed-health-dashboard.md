---
document_id: FD-B24F6B99
title: "F068 — Feed Health Dashboard"
...
# F068 — Feed Health Dashboard

## Purpose

Defines the end-to-end production requirements for the feed health dashboard feature.

## Scope

This document is the implementation authority for F068 — Feed Health Dashboard. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F068 — Feed Health Dashboard.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `feed health dashboard` workflow using versioned domain and API contracts.
...
- `feed_health_dashboard_viewed` when the workflow becomes usable.
- `feed_health_dashboard_started` after validated user intent.
- `feed_health_dashboard_completed` only after durable success.
- `feed_health_dashboard_failed` with canonical error category and retryability.
- `feed_health_dashboard_cancelled` or `abandoned` when distinguishable.
