---
document_id: FD-C757C3EB
title: "F116 — Model Fallback And Degradation"
...
# F116 — Model Fallback And Degradation

## Purpose

Defines the end-to-end production requirements for the model fallback and degradation feature.

## Scope

This document is the implementation authority for F116 — Model Fallback And Degradation. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F116 — Model Fallback And Degradation.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `model fallback and degradation` workflow using versioned domain and API contracts.
...
- `model_fallback_and_degradation_viewed` when the workflow becomes usable.
- `model_fallback_and_degradation_started` after validated user intent.
- `model_fallback_and_degradation_completed` only after durable success.
- `model_fallback_and_degradation_failed` with canonical error category and retryability.
- `model_fallback_and_degradation_cancelled` or `abandoned` when distinguishable.
