---
document_id: FD-7909879E
title: "F080 — Usage Metering"
...
Defines the end-to-end production requirements for the usage metering feature.

## Scope

This document is the implementation authority for F080 — Usage Metering. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F080 — Usage Metering.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `usage metering` workflow using versioned domain and API contracts.
...
- `usage_metering_viewed` when the workflow becomes usable.
- `usage_metering_started` after validated user intent.
- `usage_metering_completed` only after durable success.
- `usage_metering_failed` with canonical error category and retryability.
- `usage_metering_cancelled` or `abandoned` when distinguishable.
