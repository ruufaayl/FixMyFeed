---
document_id: FD-FC39FE8D
title: "F038 — Issue Status Tracking"
...
# F038 — Issue Status Tracking

## Purpose

Defines the end-to-end production requirements for the issue status tracking feature.

## Scope

This document is the implementation authority for F038 — Issue Status Tracking. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F038 — Issue Status Tracking.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `issue status tracking` workflow using versioned domain and API contracts.
...
- `issue_status_tracking_viewed` when the workflow becomes usable.
- `issue_status_tracking_started` after validated user intent.
- `issue_status_tracking_completed` only after durable success.
- `issue_status_tracking_failed` with canonical error category and retryability.
- `issue_status_tracking_cancelled` or `abandoned` when distinguishable.
