---
document_id: FD-AB1EBE4D
title: "F060 — Alert Generation"
...
# F060 — Alert Generation

## Purpose

Defines the end-to-end production requirements for the alert generation feature.

## Scope

This document is the implementation authority for F060 — Alert Generation. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F060 — Alert Generation.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `alert generation` workflow using versioned domain and API contracts.
...
- `alert_generation_viewed` when the workflow becomes usable.
- `alert_generation_started` after validated user intent.
- `alert_generation_completed` only after durable success.
- `alert_generation_failed` with canonical error category and retryability.
- `alert_generation_cancelled` or `abandoned` when distinguishable.
