---
document_id: FD-D8822F89
title: "F034 — Root Cause Analysis"
...
# F034 — Root Cause Analysis

## Purpose

Defines the end-to-end production requirements for the root cause analysis feature.

## Scope

This document is the implementation authority for F034 — Root Cause Analysis. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F034 — Root Cause Analysis.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `root cause analysis` workflow using versioned domain and API contracts.
...
- `root_cause_analysis_viewed` when the workflow becomes usable.
- `root_cause_analysis_started` after validated user intent.
- `root_cause_analysis_completed` only after durable success.
- `root_cause_analysis_failed` with canonical error category and retryability.
- `root_cause_analysis_cancelled` or `abandoned` when distinguishable.
