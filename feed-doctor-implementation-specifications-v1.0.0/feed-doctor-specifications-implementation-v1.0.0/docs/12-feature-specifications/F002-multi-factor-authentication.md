---
document_id: FD-48C7F0C3
title: "F002 — Multi Factor Authentication"
...
Defines the end-to-end production requirements for the multi factor authentication feature.

## Scope

This document is the implementation authority for F002 — Multi Factor Authentication. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F002 — Multi Factor Authentication.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `multi factor authentication` workflow using versioned domain and API contracts.
...
- `multi_factor_authentication_viewed` when the workflow becomes usable.
- `multi_factor_authentication_started` after validated user intent.
- `multi_factor_authentication_completed` only after durable success.
- `multi_factor_authentication_failed` with canonical error category and retryability.
- `multi_factor_authentication_cancelled` or `abandoned` when distinguishable.
