---
document_id: FD-0511676B
title: "F026 — Diagnostic Scan"
...
# F026 — Diagnostic Scan

## Purpose

Defines the end-to-end production requirements for the diagnostic scan feature.

## Scope

This document is the implementation authority for F026 — Diagnostic Scan. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F026 — Diagnostic Scan.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `diagnostic scan` workflow using versioned domain and API contracts.
...
- `diagnostic_scan_viewed` when the workflow becomes usable.
- `diagnostic_scan_started` after validated user intent.
- `diagnostic_scan_completed` only after durable success.
- `diagnostic_scan_failed` with canonical error category and retryability.
- `diagnostic_scan_cancelled` or `abandoned` when distinguishable.
