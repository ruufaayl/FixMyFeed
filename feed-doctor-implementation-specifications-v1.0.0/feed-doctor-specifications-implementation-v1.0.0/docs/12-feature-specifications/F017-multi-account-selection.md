---
document_id: FD-1269BF7F
title: "F017 — Multi Account Selection"
...
# F017 — Multi Account Selection

## Purpose

Defines the end-to-end production requirements for the multi account selection feature.

## Scope

This document is the implementation authority for F017 — Multi Account Selection. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F017 — Multi Account Selection.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `multi account selection` workflow using versioned domain and API contracts.
...
- `multi_account_selection_viewed` when the workflow becomes usable.
- `multi_account_selection_started` after validated user intent.
- `multi_account_selection_completed` only after durable success.
- `multi_account_selection_failed` with canonical error category and retryability.
- `multi_account_selection_cancelled` or `abandoned` when distinguishable.
