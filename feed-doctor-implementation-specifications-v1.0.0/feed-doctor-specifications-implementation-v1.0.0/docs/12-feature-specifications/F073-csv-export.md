---
document_id: FD-4971B8A5
title: "F073 — Csv Export"
...
Defines the end-to-end production requirements for the csv export feature.

## Scope

This document is the implementation authority for F073 — Csv Export. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F073 — Csv Export.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `csv export` workflow using versioned domain and API contracts.
...
- `csv_export_viewed` when the workflow becomes usable.
- `csv_export_started` after validated user intent.
- `csv_export_completed` only after durable success.
- `csv_export_failed` with canonical error category and retryability.
- `csv_export_cancelled` or `abandoned` when distinguishable.
