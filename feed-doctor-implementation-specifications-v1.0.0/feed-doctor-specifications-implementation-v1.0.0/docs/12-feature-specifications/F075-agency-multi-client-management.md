---
document_id: FD-FB4709AF
title: "F075 — Agency Multi Client Management"
...
# F075 — Agency Multi Client Management

## Purpose

Defines the end-to-end production requirements for the agency multi client management feature.

## Scope

This document is the implementation authority for F075 — Agency Multi Client Management. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F075 — Agency Multi Client Management.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `agency multi client management` workflow using versioned domain and API contracts.
...
- `agency_multi_client_management_viewed` when the workflow becomes usable.
- `agency_multi_client_management_started` after validated user intent.
- `agency_multi_client_management_completed` only after durable success.
- `agency_multi_client_management_failed` with canonical error category and retryability.
- `agency_multi_client_management_cancelled` or `abandoned` when distinguishable.
