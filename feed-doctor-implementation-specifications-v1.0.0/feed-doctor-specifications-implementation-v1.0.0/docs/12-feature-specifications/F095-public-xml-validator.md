---
document_id: FD-A1DB05CF
title: "F095 — Public Xml Validator"
...
# F095 — Public Xml Validator

## Purpose

Defines the end-to-end production requirements for the public xml validator feature.

## Scope

This document is the implementation authority for F095 — Public Xml Validator. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F095 — Public Xml Validator.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `public xml validator` workflow using versioned domain and API contracts.
...
- `public_xml_validator_viewed` when the workflow becomes usable.
- `public_xml_validator_started` after validated user intent.
- `public_xml_validator_completed` only after durable success.
- `public_xml_validator_failed` with canonical error category and retryability.
- `public_xml_validator_cancelled` or `abandoned` when distinguishable.
