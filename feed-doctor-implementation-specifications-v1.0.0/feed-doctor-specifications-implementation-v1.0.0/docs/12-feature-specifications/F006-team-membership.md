---
document_id: FD-3E72E00D
title: "F006 — Team Membership"
...
# F006 — Team Membership

## Purpose

Defines the end-to-end production requirements for the team membership feature.

## Scope

This document is the implementation authority for F006 — Team Membership. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F006 — Team Membership.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `team membership` workflow using versioned domain and API contracts.
...
- `team_membership_viewed` when the workflow becomes usable.
- `team_membership_started` after validated user intent.
- `team_membership_completed` only after durable success.
- `team_membership_failed` with canonical error category and retryability.
- `team_membership_cancelled` or `abandoned` when distinguishable.
