---
document_id: FD-F6196A82
title: "F008 — Custom Roles"
...
Defines the end-to-end production requirements for the custom roles feature.

## Scope

This document is the implementation authority for F008 — Custom Roles. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F008 — Custom Roles.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `custom roles` workflow using versioned domain and API contracts.
...
- `custom_roles_viewed` when the workflow becomes usable.
- `custom_roles_started` after validated user intent.
- `custom_roles_completed` only after durable success.
- `custom_roles_failed` with canonical error category and retryability.
- `custom_roles_cancelled` or `abandoned` when distinguishable.
