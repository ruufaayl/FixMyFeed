---
document_id: FD-6FCF6B43
title: "F044 — Repair Preview"
...
Defines the end-to-end production requirements for the repair preview feature.

## Scope

This document is the implementation authority for F044 — Repair Preview. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F044 — Repair Preview.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `repair preview` workflow using versioned domain and API contracts.
...
- `repair_preview_viewed` when the workflow becomes usable.
- `repair_preview_started` after validated user intent.
- `repair_preview_completed` only after durable success.
- `repair_preview_failed` with canonical error category and retryability.
- `repair_preview_cancelled` or `abandoned` when distinguishable.
