---
document_id: FD-6F04BADB
title: "F059 — Monitor Creation"
...
# F059 — Monitor Creation

## Purpose

Defines the end-to-end production requirements for the monitor creation feature.

## Scope

This document is the implementation authority for F059 — Monitor Creation. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F059 — Monitor Creation.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `monitor creation` workflow using versioned domain and API contracts.
...
- `monitor_creation_viewed` when the workflow becomes usable.
- `monitor_creation_started` after validated user intent.
- `monitor_creation_completed` only after durable success.
- `monitor_creation_failed` with canonical error category and retryability.
- `monitor_creation_cancelled` or `abandoned` when distinguishable.
