---
document_id: FD-11A99F1E
title: "F096 — Public Csv Validator"
...
# F096 — Public Csv Validator

## Purpose

Defines the end-to-end production requirements for the public csv validator feature.

## Scope

This document is the implementation authority for F096 — Public Csv Validator. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F096 — Public Csv Validator.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `public csv validator` workflow using versioned domain and API contracts.
...
- `public_csv_validator_viewed` when the workflow becomes usable.
- `public_csv_validator_started` after validated user intent.
- `public_csv_validator_completed` only after durable success.
- `public_csv_validator_failed` with canonical error category and retryability.
- `public_csv_validator_cancelled` or `abandoned` when distinguishable.
