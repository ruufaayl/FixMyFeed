---
document_id: FD-C5737EEC
title: "F018 — Catalog Ingestion"
...
# F018 — Catalog Ingestion

## Purpose

Defines the end-to-end production requirements for the catalog ingestion feature.

## Scope

This document is the implementation authority for F018 — Catalog Ingestion. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F018 — Catalog Ingestion.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `catalog ingestion` workflow using versioned domain and API contracts.
...
- `catalog_ingestion_viewed` when the workflow becomes usable.
- `catalog_ingestion_started` after validated user intent.
- `catalog_ingestion_completed` only after durable success.
- `catalog_ingestion_failed` with canonical error category and retryability.
- `catalog_ingestion_cancelled` or `abandoned` when distinguishable.
