---
document_id: FD-28B6A82A
title: "F004 — Scim Provisioning"
...
# F004 — Scim Provisioning

## Purpose

Defines the end-to-end production requirements for the scim provisioning feature.

## Scope

This document is the implementation authority for F004 — Scim Provisioning. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F004 — Scim Provisioning.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `scim provisioning` workflow using versioned domain and API contracts.
...
- `scim_provisioning_viewed` when the workflow becomes usable.
- `scim_provisioning_started` after validated user intent.
- `scim_provisioning_completed` only after durable success.
- `scim_provisioning_failed` with canonical error category and retryability.
- `scim_provisioning_cancelled` or `abandoned` when distinguishable.
