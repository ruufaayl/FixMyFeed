---
document_id: FD-D0889251
title: "F064 — Notification Digests"
...
# F064 — Notification Digests

## Purpose

Defines the end-to-end production requirements for the notification digests feature.

## Scope

This document is the implementation authority for F064 — Notification Digests. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F064 — Notification Digests.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `notification digests` workflow using versioned domain and API contracts.
...
- `notification_digests_viewed` when the workflow becomes usable.
- `notification_digests_started` after validated user intent.
- `notification_digests_completed` only after durable success.
- `notification_digests_failed` with canonical error category and retryability.
- `notification_digests_cancelled` or `abandoned` when distinguishable.
