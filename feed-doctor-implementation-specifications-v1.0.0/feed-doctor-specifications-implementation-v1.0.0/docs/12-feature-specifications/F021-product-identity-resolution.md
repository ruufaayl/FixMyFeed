---
document_id: FD-E173613E
title: "F021 — Product Identity Resolution"
...
# F021 — Product Identity Resolution

## Purpose

Defines the end-to-end production requirements for the product identity resolution feature.

## Scope

This document is the implementation authority for F021 — Product Identity Resolution. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F021 — Product Identity Resolution.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `product identity resolution` workflow using versioned domain and API contracts.
...
- `product_identity_resolution_viewed` when the workflow becomes usable.
- `product_identity_resolution_started` after validated user intent.
- `product_identity_resolution_completed` only after durable success.
- `product_identity_resolution_failed` with canonical error category and retryability.
- `product_identity_resolution_cancelled` or `abandoned` when distinguishable.
