---
document_id: FD-A93829C9
title: "F045 — Repair Conflict Detection"
...
# F045 — Repair Conflict Detection

## Purpose

Defines the end-to-end production requirements for the repair conflict detection feature.

## Scope

This document is the implementation authority for F045 — Repair Conflict Detection. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F045 — Repair Conflict Detection.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `repair conflict detection` workflow using versioned domain and API contracts.
...
- `repair_conflict_detection_viewed` when the workflow becomes usable.
- `repair_conflict_detection_started` after validated user intent.
- `repair_conflict_detection_completed` only after durable success.
- `repair_conflict_detection_failed` with canonical error category and retryability.
- `repair_conflict_detection_cancelled` or `abandoned` when distinguishable.
