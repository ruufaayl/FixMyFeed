---
document_id: FD-704C7D2E
title: "F066 — Slack Notifications"
...
# F066 — Slack Notifications

## Purpose

Defines the end-to-end production requirements for the slack notifications feature.

## Scope

This document is the implementation authority for F066 — Slack Notifications. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F066 — Slack Notifications.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `slack notifications` workflow using versioned domain and API contracts.
...
- `slack_notifications_viewed` when the workflow becomes usable.
- `slack_notifications_started` after validated user intent.
- `slack_notifications_completed` only after durable success.
- `slack_notifications_failed` with canonical error category and retryability.
- `slack_notifications_cancelled` or `abandoned` when distinguishable.
