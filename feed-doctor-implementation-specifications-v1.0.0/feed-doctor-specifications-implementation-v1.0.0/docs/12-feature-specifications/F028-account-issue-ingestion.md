---
document_id: FD-E99B91DC
title: "F028 — Account Issue Ingestion"
...
# F028 — Account Issue Ingestion

## Purpose

Defines the end-to-end production requirements for the account issue ingestion feature.

## Scope

This document is the implementation authority for F028 — Account Issue Ingestion. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F028 — Account Issue Ingestion.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `account issue ingestion` workflow using versioned domain and API contracts.
...
- `account_issue_ingestion_viewed` when the workflow becomes usable.
- `account_issue_ingestion_started` after validated user intent.
- `account_issue_ingestion_completed` only after durable success.
- `account_issue_ingestion_failed` with canonical error category and retryability.
- `account_issue_ingestion_cancelled` or `abandoned` when distinguishable.
