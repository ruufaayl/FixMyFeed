---
document_id: FD-F6FA02A0
title: "F011 — Guided Onboarding"
...
# F011 — Guided Onboarding

## Purpose

Defines the end-to-end production requirements for the guided onboarding feature.

## Scope

This document is the implementation authority for F011 — Guided Onboarding. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F011 — Guided Onboarding.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `guided onboarding` workflow using versioned domain and API contracts.
...
- `guided_onboarding_viewed` when the workflow becomes usable.
- `guided_onboarding_started` after validated user intent.
- `guided_onboarding_completed` only after durable success.
- `guided_onboarding_failed` with canonical error category and retryability.
- `guided_onboarding_cancelled` or `abandoned` when distinguishable.
