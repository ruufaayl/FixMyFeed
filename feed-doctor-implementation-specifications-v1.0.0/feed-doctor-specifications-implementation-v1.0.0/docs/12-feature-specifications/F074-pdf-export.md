---
document_id: FD-5B8CF03A
title: "F074 — Pdf Export"
...
Defines the end-to-end production requirements for the pdf export feature.

## Scope

This document is the implementation authority for F074 — Pdf Export. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F074 — Pdf Export.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `pdf export` workflow using versioned domain and API contracts.
...
- `pdf_export_viewed` when the workflow becomes usable.
- `pdf_export_started` after validated user intent.
- `pdf_export_completed` only after durable success.
- `pdf_export_failed` with canonical error category and retryability.
- `pdf_export_cancelled` or `abandoned` when distinguishable.
