---
document_id: FD-936C8D86
title: "F042 — Single Product Repair"
...
# F042 — Single Product Repair

## Purpose

Defines the end-to-end production requirements for the single product repair feature.

## Scope

This document is the implementation authority for F042 — Single Product Repair. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F042 — Single Product Repair.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `single product repair` workflow using versioned domain and API contracts.
...
- `single_product_repair_viewed` when the workflow becomes usable.
- `single_product_repair_started` after validated user intent.
- `single_product_repair_completed` only after durable success.
- `single_product_repair_failed` with canonical error category and retryability.
- `single_product_repair_cancelled` or `abandoned` when distinguishable.
