---
document_id: FD-A7988D6F
title: "F016 — Merchant Account Discovery"
...
# F016 — Merchant Account Discovery

## Purpose

Defines the end-to-end production requirements for the merchant account discovery feature.

## Scope

This document is the implementation authority for F016 — Merchant Account Discovery. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F016 — Merchant Account Discovery.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `merchant account discovery` workflow using versioned domain and API contracts.
...
- `merchant_account_discovery_viewed` when the workflow becomes usable.
- `merchant_account_discovery_started` after validated user intent.
- `merchant_account_discovery_completed` only after durable success.
- `merchant_account_discovery_failed` with canonical error category and retryability.
- `merchant_account_discovery_cancelled` or `abandoned` when distinguishable.
