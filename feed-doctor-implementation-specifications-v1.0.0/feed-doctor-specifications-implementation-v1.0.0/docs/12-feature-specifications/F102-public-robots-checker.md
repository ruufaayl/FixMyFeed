---
document_id: FD-7DE9A44A
title: "F102 — Public Robots Checker"
...
# F102 — Public Robots Checker

## Purpose

Defines the end-to-end production requirements for the public robots checker feature.

## Scope

This document is the implementation authority for F102 — Public Robots Checker. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F102 — Public Robots Checker.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `public robots checker` workflow using versioned domain and API contracts.
...
- `public_robots_checker_viewed` when the workflow becomes usable.
- `public_robots_checker_started` after validated user intent.
- `public_robots_checker_completed` only after durable success.
- `public_robots_checker_failed` with canonical error category and retryability.
- `public_robots_checker_cancelled` or `abandoned` when distinguishable.
