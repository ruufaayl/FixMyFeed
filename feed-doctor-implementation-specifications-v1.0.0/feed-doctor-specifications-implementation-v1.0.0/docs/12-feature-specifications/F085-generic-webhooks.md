---
document_id: FD-677B92C9
title: "F085 — Generic Webhooks"
...
# F085 — Generic Webhooks

## Purpose

Defines the end-to-end production requirements for the generic webhooks feature.

## Scope

This document is the implementation authority for F085 — Generic Webhooks. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F085 — Generic Webhooks.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `generic webhooks` workflow using versioned domain and API contracts.
...
- `generic_webhooks_viewed` when the workflow becomes usable.
- `generic_webhooks_started` after validated user intent.
- `generic_webhooks_completed` only after durable success.
- `generic_webhooks_failed` with canonical error category and retryability.
- `generic_webhooks_cancelled` or `abandoned` when distinguishable.
