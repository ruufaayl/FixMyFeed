---
document_id: FD-08D5F22D
title: "F104 — Public Category Finder"
...
# F104 — Public Category Finder

## Purpose

Defines the end-to-end production requirements for the public category finder feature.

## Scope

This document is the implementation authority for F104 — Public Category Finder. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F104 — Public Category Finder.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `public category finder` workflow using versioned domain and API contracts.
...
- `public_category_finder_viewed` when the workflow becomes usable.
- `public_category_finder_started` after validated user intent.
- `public_category_finder_completed` only after durable success.
- `public_category_finder_failed` with canonical error category and retryability.
- `public_category_finder_cancelled` or `abandoned` when distinguishable.
