---
document_id: FD-74F02F60
title: "F058 — Rule Assignment"
...
# F058 — Rule Assignment

## Purpose

Defines the end-to-end production requirements for the rule assignment feature.

## Scope

This document is the implementation authority for F058 — Rule Assignment. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F058 — Rule Assignment.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `rule assignment` workflow using versioned domain and API contracts.
...
- `rule_assignment_viewed` when the workflow becomes usable.
- `rule_assignment_started` after validated user intent.
- `rule_assignment_completed` only after durable success.
- `rule_assignment_failed` with canonical error category and retryability.
- `rule_assignment_cancelled` or `abandoned` when distinguishable.
