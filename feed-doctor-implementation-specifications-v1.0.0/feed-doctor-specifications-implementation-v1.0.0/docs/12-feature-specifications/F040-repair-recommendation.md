---
document_id: FD-A87375C2
title: "F040 — Repair Recommendation"
...
# F040 — Repair Recommendation

## Purpose

Defines the end-to-end production requirements for the repair recommendation feature.

## Scope

This document is the implementation authority for F040 — Repair Recommendation. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F040 — Repair Recommendation.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `repair recommendation` workflow using versioned domain and API contracts.
...
- `repair_recommendation_viewed` when the workflow becomes usable.
- `repair_recommendation_started` after validated user intent.
- `repair_recommendation_completed` only after durable success.
- `repair_recommendation_failed` with canonical error category and retryability.
- `repair_recommendation_cancelled` or `abandoned` when distinguishable.
