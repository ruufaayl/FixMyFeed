---
document_id: FD-E42F8CDC
title: "F105 — Public Policy Readiness Checker"
...
# F105 — Public Policy Readiness Checker

## Purpose

Defines the end-to-end production requirements for the public policy readiness checker feature.

## Scope

This document is the implementation authority for F105 — Public Policy Readiness Checker. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F105 — Public Policy Readiness Checker.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `public policy readiness checker` workflow using versioned domain and API contracts.
...
- Anonymous use is quota-limited and protected against SSRF, oversized input, automation abuse, and stored-content injection.
- Indexability requires a useful static explanation or tool interface; ephemeral results are noindex.
...
- `public_policy_readiness_checker_viewed` when the workflow becomes usable.
- `public_policy_readiness_checker_started` after validated user intent.
- `public_policy_readiness_checker_completed` only after durable success.
- `public_policy_readiness_checker_failed` with canonical error category and retryability.
- `public_policy_readiness_checker_cancelled` or `abandoned` when distinguishable.
