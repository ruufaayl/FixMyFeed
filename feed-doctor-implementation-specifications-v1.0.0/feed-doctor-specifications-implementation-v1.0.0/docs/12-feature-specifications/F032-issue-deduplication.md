---
document_id: FD-811FF91D
title: "F032 — Issue Deduplication"
...
# F032 — Issue Deduplication

## Purpose

Defines the end-to-end production requirements for the issue deduplication feature.

## Scope

This document is the implementation authority for F032 — Issue Deduplication. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F032 — Issue Deduplication.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `issue deduplication` workflow using versioned domain and API contracts.
...
- `issue_deduplication_viewed` when the workflow becomes usable.
- `issue_deduplication_started` after validated user intent.
- `issue_deduplication_completed` only after durable success.
- `issue_deduplication_failed` with canonical error category and retryability.
- `issue_deduplication_cancelled` or `abandoned` when distinguishable.
