---
document_id: FD-77AE7E6E
title: "F088 — Localization"
...
Defines the end-to-end production requirements for the localization feature.

## Scope

This document is the implementation authority for F088 — Localization. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F088 — Localization.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `localization` workflow using versioned domain and API contracts.
...
- `localization_viewed` when the workflow becomes usable.
- `localization_started` after validated user intent.
- `localization_completed` only after durable success.
- `localization_failed` with canonical error category and retryability.
- `localization_cancelled` or `abandoned` when distinguishable.
