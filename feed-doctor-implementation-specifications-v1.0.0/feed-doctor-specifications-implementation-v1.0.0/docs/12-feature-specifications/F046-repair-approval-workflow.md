---
document_id: FD-9890EDDE
title: "F046 — Repair Approval Workflow"
...
# F046 — Repair Approval Workflow

## Purpose

Defines the end-to-end production requirements for the repair approval workflow feature.

## Scope

This document is the implementation authority for F046 — Repair Approval Workflow. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F046 — Repair Approval Workflow.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `repair approval workflow` workflow using versioned domain and API contracts.
...
- `repair_approval_workflow_viewed` when the workflow becomes usable.
- `repair_approval_workflow_started` after validated user intent.
- `repair_approval_workflow_completed` only after durable success.
- `repair_approval_workflow_failed` with canonical error category and retryability.
- `repair_approval_workflow_cancelled` or `abandoned` when distinguishable.
