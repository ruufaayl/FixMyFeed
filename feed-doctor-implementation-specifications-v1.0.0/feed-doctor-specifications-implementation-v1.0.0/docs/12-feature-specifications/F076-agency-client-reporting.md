---
document_id: FD-77A87163
title: "F076 — Agency Client Reporting"
...
# F076 — Agency Client Reporting

## Purpose

Defines the end-to-end production requirements for the agency client reporting feature.

## Scope

This document is the implementation authority for F076 — Agency Client Reporting. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F076 — Agency Client Reporting.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `agency client reporting` workflow using versioned domain and API contracts.
...
- `agency_client_reporting_viewed` when the workflow becomes usable.
- `agency_client_reporting_started` after validated user intent.
- `agency_client_reporting_completed` only after durable success.
- `agency_client_reporting_failed` with canonical error category and retryability.
- `agency_client_reporting_cancelled` or `abandoned` when distinguishable.
