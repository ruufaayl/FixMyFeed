---
document_id: FD-0368C01F
title: "F031 — Issue Normalization"
...
# F031 — Issue Normalization

## Purpose

Defines the end-to-end production requirements for the issue normalization feature.

## Scope

This document is the implementation authority for F031 — Issue Normalization. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F031 — Issue Normalization.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `issue normalization` workflow using versioned domain and API contracts.
...
- `issue_normalization_viewed` when the workflow becomes usable.
- `issue_normalization_started` after validated user intent.
- `issue_normalization_completed` only after durable success.
- `issue_normalization_failed` with canonical error category and retryability.
- `issue_normalization_cancelled` or `abandoned` when distinguishable.
