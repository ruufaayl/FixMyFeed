---
document_id: FD-FC6E7B9C
title: "F063 — Notification Routing"
...
# F063 — Notification Routing

## Purpose

Defines the end-to-end production requirements for the notification routing feature.

## Scope

This document is the implementation authority for F063 — Notification Routing. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F063 — Notification Routing.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `notification routing` workflow using versioned domain and API contracts.
...
- `notification_routing_viewed` when the workflow becomes usable.
- `notification_routing_started` after validated user intent.
- `notification_routing_completed` only after durable success.
- `notification_routing_failed` with canonical error category and retryability.
- `notification_routing_cancelled` or `abandoned` when distinguishable.
