---
document_id: FD-301F2630
title: "F041 — Manual Repair Instructions"
...
# F041 — Manual Repair Instructions

## Purpose

Defines the end-to-end production requirements for the manual repair instructions feature.

## Scope

This document is the implementation authority for F041 — Manual Repair Instructions. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of F041 — Manual Repair Instructions.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The feature MUST implement the complete `manual repair instructions` workflow using versioned domain and API contracts.
...
- `manual_repair_instructions_viewed` when the workflow becomes usable.
- `manual_repair_instructions_started` after validated user intent.
- `manual_repair_instructions_completed` only after durable success.
- `manual_repair_instructions_failed` with canonical error category and retryability.
- `manual_repair_instructions_cancelled` or `abandoned` when distinguishable.
