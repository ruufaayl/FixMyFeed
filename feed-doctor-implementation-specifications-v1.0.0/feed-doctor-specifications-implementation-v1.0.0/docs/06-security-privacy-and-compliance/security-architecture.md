...
Defines the trust model, controls, key management, credential handling, tenant isolation, incident boundaries, and release gates.

## Scope

This document is the implementation authority for Security Architecture. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Security Architecture.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Authentication uses Better Auth with secure HTTP-only cookies, rotating session identifiers, verified email, optional TOTP MFA, and enterprise OIDC/SAML only through approved extensions.
- Authorization combines organization-scoped RBAC with resource ownership checks. Every repository method requires an explicit tenant context.
- Connector access and refresh tokens are encrypted using envelope encryption before persistence and decrypted only inside server or worker processes.
- High-risk operations require recent authentication, explicit permission, change preview, and immutable audit logging.
- External webhooks require provider-specific signature verification, timestamp or replay checks, deduplication, and body-size limits.
- Public tools run in a restricted execution path with no tenant data access, strict input limits, SSRF protections, and temporary retention.
- Support access is time-boxed, customer-approved, least-privileged, reason-bound, and recorded.
- Security controls must work without a paid WAF or security API; optional providers may strengthen but never replace application controls.
...
The primary threats are cross-tenant data exposure, stolen OAuth tokens, forged webhooks, SSRF through feed and image URLs, malicious oversized feeds, unsafe automated repair, account takeover, export abuse, dependency compromise, and administrator misuse.

## Encryption Model

A master data-encryption key is supplied as an environment secret. Each credential record is encrypted with authenticated encryption and contextual associated data including organization, connector, and record ID. Key rotation supports previous-key decryption and current-key encryption. Database backups never contain plaintext access tokens.

## Release Blocking Controls

Tenant-isolation tests, authorization matrix tests, webhook signature tests, SSRF tests, secret scanning, dependency scanning, audit-log coverage, and restore tests must pass before production.
