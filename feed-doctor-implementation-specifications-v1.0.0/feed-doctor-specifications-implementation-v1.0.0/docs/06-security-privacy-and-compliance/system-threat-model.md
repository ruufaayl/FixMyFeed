...
Enumerates assets, actors, attack surfaces, abuse cases, mitigations, residual risks, and verification evidence.

## Scope

This document is the implementation authority for System Threat Model. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of System Threat Model.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Apply STRIDE to web, worker, database, object storage, connectors, webhooks, public tools, and administrative surfaces.
- Model malicious tenants, unauthenticated attackers, compromised upstream platforms, insiders, dependency attackers, and accidental operators.
- Every high or critical threat requires preventive, detective, and recovery controls plus a named test or runbook.
- Threats introduced by a change must update this document before merge.
...
## High-Risk Threat Register

| Threat | Impact | Required Controls |
|---|---|---|
| Cross-tenant query | Critical | Tenant-aware repositories, composite constraints, authorization tests. |
| OAuth token theft | Critical | Encryption, redaction, scoped tokens, rotation, revocation monitoring. |
| SSRF from remote feed or image | Critical | URL parser, DNS/IP checks, redirect revalidation, private-range denylist, byte/time limits. |
| Forged webhook | High | Raw-body signature verification, replay window, deduplication. |
| Unsafe writeback | High | Safety class, preview, approval, idempotency, verification, rollback. |
| Resource exhaustion | High | quotas, streaming, fair scheduling, cancellation, hard limits. |
| SEO spam injection | High | editorial workflow, sanitization, structured content model, no raw HTML by default. |
