...
Defines retention classes, deletion workflows, legal holds, connector revocation, backups, and verifiable purge.

## Scope

This document is the implementation authority for Data Retention and Deletion. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Data Retention and Deletion.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Temporary public-tool inputs: maximum 24 hours unless explicitly saved.
- Raw import files: default 7 days after successful normalization; configurable to 30 days for paid plans later.
- Product source snapshots: 30 days bootstrap; aggregate issue history retained longer without raw payload.
- Exports: 7 days bootstrap.
- Audit and security events: minimum 365 days where legally permitted.
- Deleted tenant data enters a scheduled purge workflow with connector revocation and object deletion.
- Backups expire through provider lifecycle and are not selectively edited; restoration procedures reapply deletion tombstones.
- Legal holds suspend purge only for scoped records and require audited authorization.
