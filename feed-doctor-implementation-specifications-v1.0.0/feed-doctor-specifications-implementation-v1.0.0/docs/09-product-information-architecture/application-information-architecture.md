...
Defines route hierarchy, navigation, object context, switching, global search, saved views, and one-workflow-per-page enforcement.

## Scope

This document is the implementation authority for Application Information Architecture. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Application Information Architecture.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Primary hierarchy: organization → workspace → store → merchant account or catalog → workflow object.
- Every page has one primary user goal and no more than one primary call to action.
- Context switchers preserve only filters valid in the destination context.
- Deep links are stable and authorization-aware.
- Global search returns only authorized stores, products, issues, repairs, rules, monitors, and documentation.
- Destructive or high-risk operations never originate from ambiguous bulk selection.
- Browser back/forward navigation restores safe view state without repeating mutations.
