---
document_id: FD-2C95D4E3
title: "Issue Evidence Model"
...
Defines the domain model for Evidence is immutable, source-attributed, time-stamped, redacted, and sufficient to reproduce a finding.

## Scope

This document is the implementation authority for Issue Evidence Model. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Issue Evidence Model.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Evidence is immutable, source-attributed, time-stamped, redacted, and sufficient to reproduce a finding.
...
The owning aggregate for `issue-evidence-model` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
