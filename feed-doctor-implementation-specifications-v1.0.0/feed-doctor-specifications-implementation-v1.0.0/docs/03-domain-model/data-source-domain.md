...
Defines the domain model for Data sources describe platform APIs, uploaded files, scheduled URLs, SFTP, or destination-managed sources.

## Scope

This document is the implementation authority for Data Source Domain. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Data Source Domain.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Data sources describe platform APIs, uploaded files, scheduled URLs, SFTP, or destination-managed sources.
...
The owning aggregate for `data-source-domain` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
