...
Defines the domain model for Policy knowledge records authoritative provider sources and guidance but does not make legal or provider-approval guarantees.

## Scope

This document is the implementation authority for Policy Domain. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Policy Domain.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Policy knowledge records authoritative provider sources and guidance but does not make legal or provider-approval guarantees.
...
The owning aggregate for `policy-domain` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
