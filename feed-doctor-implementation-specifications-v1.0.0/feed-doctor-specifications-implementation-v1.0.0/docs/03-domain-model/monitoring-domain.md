...
Defines the domain model for Monitors define targets, schedules, thresholds, baseline, suppression, and escalation.

## Scope

This document is the implementation authority for Monitoring Domain. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Monitoring Domain.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Monitors define targets, schedules, thresholds, baseline, suppression, and escalation.
...
The owning aggregate for `monitoring-domain` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
