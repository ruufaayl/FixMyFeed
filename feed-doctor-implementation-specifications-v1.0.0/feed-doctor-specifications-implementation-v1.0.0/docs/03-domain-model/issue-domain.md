...
Defines the domain model for Issue instances represent a normalized problem affecting entities, destinations, eligibility, or account state.

## Scope

This document is the implementation authority for Issue Domain. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Issue Domain.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Issue instances represent a normalized problem affecting entities, destinations, eligibility, or account state.
...
The owning aggregate for `issue-domain` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
