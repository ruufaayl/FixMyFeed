---
document_id: FD-7241E5F1
title: "Issue Severity Model"
...
Defines the domain model for Severity is based on eligibility and operational impact, not UI color or provider wording alone.

## Scope

This document is the implementation authority for Issue Severity Model. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Issue Severity Model.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Severity is based on eligibility and operational impact, not UI color or provider wording alone.
...
The owning aggregate for `issue-severity-model` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
