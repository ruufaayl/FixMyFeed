...
Defines the domain model for Feed is a versioned product-data projection with mappings, schedule, market, destination, and lifecycle.

## Scope

This document is the implementation authority for Feed Domain. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Feed Domain.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Feed is a versioned product-data projection with mappings, schedule, market, destination, and lifecycle.
...
The owning aggregate for `feed-domain` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
