...
Defines the domain model for The system is transactionally consistent internally and explicitly eventually consistent with external platforms.

## Scope

This document is the implementation authority for Consistency Model. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Consistency Model.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The system is transactionally consistent internally and explicitly eventually consistent with external platforms.
...
The owning aggregate for `consistency-model` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
