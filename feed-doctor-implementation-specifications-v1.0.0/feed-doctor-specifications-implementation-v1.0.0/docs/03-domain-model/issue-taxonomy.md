...
Defines the domain model for Issue definitions classify source aliases, scope, severity, evidence, causes, remediations, SEO knowledge, and version history.

## Scope

This document is the implementation authority for Issue Taxonomy. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Issue Taxonomy.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Issue definitions classify source aliases, scope, severity, evidence, causes, remediations, SEO knowledge, and version history.
...
The owning aggregate for `issue-taxonomy` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
