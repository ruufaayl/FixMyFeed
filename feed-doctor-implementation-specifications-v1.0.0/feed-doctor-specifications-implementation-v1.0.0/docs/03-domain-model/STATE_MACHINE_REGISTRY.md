---
document_id: FD-BCC38B50
title: "State Machine Registry"
...
Defines canonical lifecycle states and permitted transitions for all long-lived resources.

## Scope

This document is the implementation authority for State Machine Registry. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of State Machine Registry.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- State names are lowercase machine enums and have one user-facing localization mapping.
- Transitions are validated centrally and recorded with actor, time, reason, and expected version.
- Retries create attempt records and do not move terminal resources backwards.
- Unknown provider states map to explicit unknown/degraded classifications rather than existing success states.
...
`draft → authorizing → active | degraded | revoked | disconnected`

### Sync Run

`queued → acquiring → parsing → normalizing → reconciling → completed | partial | failed | cancelled`

### Scan

`queued → preparing → evaluating → aggregating → completed | partial | failed | cancelled`

### Issue

`detected → active → acknowledged | suppressed → repair_planned → verification_pending → resolved | regressed | retired`

### Repair Plan

`draft → previewed → awaiting_approval → approved | rejected | expired → executing → verifying → completed | partial | failed → rollback_pending | rolled_back`

### Job

`created → queued → active → retry_scheduled → completed | failed | cancelled | dead_letter`

### Alert

`open → acknowledged → investigating → resolved | suppressed | expired`

### Content Page

`draft → technical_review → editorial_review → approved → indexable | noindex → retired | redirected`

### Subscription

`inactive → trialing | active → past_due → grace → suspended | cancelled`

### Deletion Request

`requested → verified → scheduled → purging → completed | blocked_by_hold | failed`
