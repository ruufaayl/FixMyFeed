---
document_id: FD-0692A3FD
title: "Remediation Domain"
...
Defines the domain model for Remediation definitions map issues to safe options, authority, fields, dependencies, verification, and rollback.

## Scope

This document is the implementation authority for Remediation Domain. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Remediation Domain.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Remediation definitions map issues to safe options, authority, fields, dependencies, verification, and rollback.
...
The owning aggregate for `remediation-domain` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
