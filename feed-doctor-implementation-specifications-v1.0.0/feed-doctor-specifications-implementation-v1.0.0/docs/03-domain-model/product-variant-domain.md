---
document_id: FD-B7EBFB12
title: "Product Variant Domain"
...
Defines the domain model for Variant is a purchasable product configuration with stable source identity, option set, price, availability, and media.

## Scope

This document is the implementation authority for Product Variant Domain. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Product Variant Domain.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Variant is a purchasable product configuration with stable source identity, option set, price, availability, and media.
...
The owning aggregate for `product-variant-domain` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
