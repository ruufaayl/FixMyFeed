---
document_id: FD-36E1A133
title: "Issue Status Lifecycle"
...
Defines the domain model for Issues move through detected, active, acknowledged, suppressed, repair-planned, verification-pending, resolved, regressed, and retired states.

## Scope

This document is the implementation authority for Issue Status Lifecycle. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Issue Status Lifecycle.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Issues move through detected, active, acknowledged, suppressed, repair-planned, verification-pending, resolved, regressed, and retired states.
...
The owning aggregate for `issue-status-lifecycle` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
