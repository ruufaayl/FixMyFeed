...
Defines the domain model for Defines non-negotiable cross-domain rules such as tenant isolation, immutable evidence, no issue pass from missing data, and no writeback without approval.

## Scope

This document is the implementation authority for Domain Invariants. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Domain Invariants.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Defines non-negotiable cross-domain rules such as tenant isolation, immutable evidence, no issue pass from missing data, and no writeback without approval.
...
The owning aggregate for `domain-invariants` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
