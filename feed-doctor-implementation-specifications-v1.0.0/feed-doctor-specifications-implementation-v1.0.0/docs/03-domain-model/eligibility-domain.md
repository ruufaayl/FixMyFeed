---
document_id: FD-0A62C738
title: "Eligibility Domain"
...
Defines the domain model for Eligibility records product and account ability to serve by destination and market, including unknown and processing states.

## Scope

This document is the implementation authority for Eligibility Domain. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Eligibility Domain.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Eligibility records product and account ability to serve by destination and market, including unknown and processing states.
...
The owning aggregate for `eligibility-domain` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
