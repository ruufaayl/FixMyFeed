---
document_id: FD-035FBB7E
title: "Issue Impact Model"
...
Defines the domain model for Impact combines affected products, destinations, market scope, revenue proxy, recurrence, and confidence without claiming causal revenue certainty.

## Scope

This document is the implementation authority for Issue Impact Model. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Issue Impact Model.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Impact combines affected products, destinations, market scope, revenue proxy, recurrence, and confidence without claiming causal revenue certainty.
...
The owning aggregate for `issue-impact-model` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
