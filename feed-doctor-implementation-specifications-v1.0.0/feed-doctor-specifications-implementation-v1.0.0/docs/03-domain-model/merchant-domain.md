...
Defines the domain model for Merchant represents the commercial operator linked to one or more stores and destination accounts without assuming one-to-one relationships.

## Scope

This document is the implementation authority for Merchant Domain. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Merchant Domain.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Merchant represents the commercial operator linked to one or more stores and destination accounts without assuming one-to-one relationships.
...
The owning aggregate for `merchant-domain` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
