...
Defines the domain model for Audit records immutable security and business-significant actions, actors, scope, evidence, and outcome.

## Scope

This document is the implementation authority for Audit Domain. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Audit Domain.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Audit records immutable security and business-significant actions, actors, scope, evidence, and outcome.
...
The owning aggregate for `audit-domain` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
