---
document_id: FD-58AD2A13
title: "Rule Engine Domain"
...
Defines the domain model for Rules transform or validate scoped catalog data using a versioned deterministic expression language.

## Scope

This document is the implementation authority for Rule Engine Domain. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Rule Engine Domain.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Rules transform or validate scoped catalog data using a versioned deterministic expression language.
...
The owning aggregate for `rule-engine-domain` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
