---
document_id: FD-EEC2A05D
title: "Remediation Safety Classes"
...
# Remediation Safety Classes

## Purpose

Defines the domain model for Safety classes separate explanation, instructions, reversible proposals, approved writebacks, and provider-controlled actions.

## Scope

This document is the implementation authority for Remediation Safety Classes. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Remediation Safety Classes.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Safety classes separate explanation, instructions, reversible proposals, approved writebacks, and provider-controlled actions.
...
The owning aggregate for `remediation-safety-classes` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
