...
Defines the domain model for Notifications are rendered delivery attempts derived from domain events and user preferences.

## Scope

This document is the implementation authority for Notification Domain. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Notification Domain.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Notifications are rendered delivery attempts derived from domain events and user preferences.
...
The owning aggregate for `notification-domain` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
