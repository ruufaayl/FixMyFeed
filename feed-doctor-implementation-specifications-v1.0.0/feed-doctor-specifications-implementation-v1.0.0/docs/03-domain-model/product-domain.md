...
Defines the domain model for Product is the parent commercial item with variants, identities, attributes, media, markets, snapshots, and mappings.

## Scope

This document is the implementation authority for Product Domain. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Product Domain.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Product is the parent commercial item with variants, identities, attributes, media, markets, snapshots, and mappings.
...
The owning aggregate for `product-domain` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
