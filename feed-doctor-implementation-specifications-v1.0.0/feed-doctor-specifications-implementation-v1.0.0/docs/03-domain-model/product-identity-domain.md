---
document_id: FD-2C0203FD
title: "Product Identity Domain"
...
Defines the domain model for Identity resolves source and destination records using explicit provider IDs, GTIN, MPN, SKU, item group, and evidence-backed fallback.

## Scope

This document is the implementation authority for Product Identity Domain. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Product Identity Domain.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Identity resolves source and destination records using explicit provider IDs, GTIN, MPN, SKU, item group, and evidence-backed fallback.
...
The owning aggregate for `product-identity-domain` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
