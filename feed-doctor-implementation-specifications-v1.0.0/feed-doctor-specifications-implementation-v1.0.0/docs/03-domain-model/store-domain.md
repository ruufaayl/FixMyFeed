...
Defines the domain model for Store is a source-commerce boundary with platform, domains, markets, catalogs, credentials, and connection health.

## Scope

This document is the implementation authority for Store Domain. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Store Domain.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Store is a source-commerce boundary with platform, domains, markets, catalogs, credentials, and connection health.
...
The owning aggregate for `store-domain` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
