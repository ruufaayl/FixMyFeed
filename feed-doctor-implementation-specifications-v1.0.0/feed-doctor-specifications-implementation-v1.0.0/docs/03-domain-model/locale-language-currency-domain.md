---
document_id: FD-D1348BE3
title: "Locale Language Currency Domain"
...
# Locale Language Currency Domain

## Purpose

Defines the domain model for Locale, language, currency, units, and market are explicit context and never guessed from display alone.

## Scope

This document is the implementation authority for Locale Language Currency Domain. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Locale Language Currency Domain.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Locale, language, currency, units, and market are explicit context and never guessed from display alone.
...
The owning aggregate for `locale-language-currency-domain` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
