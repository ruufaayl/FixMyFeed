---
document_id: FD-677C5C9A
title: "Source Of Truth Resolution"
...
Defines the domain model for Authority selects which platform or feed controls each attribute before any repair is proposed.

## Scope

This document is the implementation authority for Source Of Truth Resolution. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Source Of Truth Resolution.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Authority selects which platform or feed controls each attribute before any repair is proposed.
...
The owning aggregate for `source-of-truth-resolution` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
