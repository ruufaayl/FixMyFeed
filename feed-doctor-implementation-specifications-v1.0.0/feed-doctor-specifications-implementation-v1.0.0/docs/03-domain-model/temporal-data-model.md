...
Defines the domain model for Snapshots, effective times, observed times, processed times, and user-display times remain distinct.

## Scope

This document is the implementation authority for Temporal Data Model. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Temporal Data Model.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Snapshots, effective times, observed times, processed times, and user-display times remain distinct.
...
The owning aggregate for `temporal-data-model` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
