...
Defines the domain model for Catalog is a scoped collection of normalized products for a store, source, market, or destination.

## Scope

This document is the implementation authority for Catalog Domain. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Catalog Domain.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Catalog is a scoped collection of normalized products for a store, source, market, or destination.
...
The owning aggregate for `catalog-domain` is modified through one application service and tenant-aware repository. Related records outside the aggregate are referenced by ID or coordinated asynchronously.
