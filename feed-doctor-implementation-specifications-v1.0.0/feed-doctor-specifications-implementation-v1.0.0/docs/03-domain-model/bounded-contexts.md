...
Defines domain boundaries, ownership, public language, integrations, and anti-corruption layers.

## Scope

This document is the implementation authority for Bounded Contexts. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Bounded Contexts.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Contexts: Identity and Tenancy; Connections; Catalog; Feed Processing; Diagnostics; Issue Registry; Repair and Approval; Rules; Monitoring and Notifications; Billing and Entitlements; Public Content and SEO; Audit and Compliance; Platform Operations.
- Each aggregate has one owning context and one write authority.
- Cross-context communication uses versioned service interfaces or immutable events.
- External provider models are translated through connector anti-corruption layers.
- UI composition may read multiple contexts but may not create cross-context transactions.
...
Identity supplies actor and tenant context. Connections owns credentials and provider capability. Catalog owns normalized products. Feed Processing owns imports/syncs/snapshots. Diagnostics consumes snapshots and emits findings. Issue Registry owns issue definitions. Repair consumes issues and emits approved change sets. Connectors execute external effects. Monitoring consumes events and schedules scans. SEO consumes reviewed issue knowledge but never tenant data.
