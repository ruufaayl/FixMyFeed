...
Defines an optional self-hosted model adapter for non-authoritative drafting and classification assistance.

## Scope

This document is the implementation authority for Local Model Adapter. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Local Model Adapter.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Adapter is disabled by default and configured only by private base URL and model identifier.
- Requests have strict byte, token, time, concurrency, and retry limits.
- Prompts exclude secrets and minimize catalog data.
- Responses must match a versioned JSON schema and pass deterministic validation.
- Timeout, malformed output, or model unavailability returns an optional-assistance-unavailable state without blocking core workflows.
