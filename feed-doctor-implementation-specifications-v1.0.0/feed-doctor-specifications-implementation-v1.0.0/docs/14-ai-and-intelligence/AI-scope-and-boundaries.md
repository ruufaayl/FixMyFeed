---
document_id: FD-456B4D02
title: "AI Scope and Boundaries"
...
Defines optional intelligence features while guaranteeing that the production core works without paid model APIs.

## Scope

This document is the implementation authority for AI Scope and Boundaries. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of AI Scope and Boundaries.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Core catalog ingestion, validation, issue explanation templates, repair eligibility, change generation, authorization, and verification are deterministic.
- AI features are disabled by default and cannot create or clear authoritative issues.
- Supported zero-mandatory-cost mode is a local OpenAI-compatible or Ollama adapter operated by the owner.
- No customer data is sent to an external model provider unless a future explicit opt-in adapter and DPA are approved.
- AI output is schema-constrained, provenance-labeled, validated, and reviewed before any mutation.
- System behavior remains complete when AI is unavailable.
