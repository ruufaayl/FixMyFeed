---
document_id: FD-6B6EF74C
title: "First-Party Analytics Architecture"
...
Defines consent-aware product and SEO analytics without requiring a paid analytics API.

## Scope

This document is the implementation authority for First-Party Analytics Architecture. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of First-Party Analytics Architecture.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Capture approved events through the application API into a bounded PostgreSQL event table or aggregate counters.
- Separate operational telemetry from product analytics and audit logs.
- Anonymous identifiers are first-party, rotating, and not used for cross-site tracking.
- Consent requirements are evaluated by jurisdiction and page surface.
- Raw event retention is short; durable metrics are aggregated.
- Event ingestion applies schema validation, sampling where approved, rate limits, and PII rejection.
- Exports to an external warehouse remain optional and adapter-based.
