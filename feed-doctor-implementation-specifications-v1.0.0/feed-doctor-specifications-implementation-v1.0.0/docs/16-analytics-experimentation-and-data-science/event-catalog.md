...
Defines canonical product, operational, acquisition, conversion, and safety event names and properties.

## Scope

This document is the implementation authority for Analytics Event Catalog. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Analytics Event Catalog.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Use past-tense event names and versioned schemas.
- Common properties: event ID, occurred time, anonymous/user ID, organization/workspace/store IDs where authorized, surface, feature, outcome, duration bucket, error category, experiment assignments, and consent class.
- Never capture raw titles, descriptions, feed rows, credentials, full email addresses, payment data, or URL query strings.
