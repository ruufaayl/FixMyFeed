---
document_id: FD-01BB7DC1
title: "Page Specification — Catalog List"
...
# Page Specification — Catalog List

## Purpose

Defines the complete user experience and implementation contract for the catalog list workflow.

## Scope

This document is the implementation authority for Page Specification — Catalog List. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `catalog list` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: catalog list.
- Canonical route: `/app/catalog-list`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=catalog-list` and authorized context classification.
- `catalog_list_primary_action_started`.
