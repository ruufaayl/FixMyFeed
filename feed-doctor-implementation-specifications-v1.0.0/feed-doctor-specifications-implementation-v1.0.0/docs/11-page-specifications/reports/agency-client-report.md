---
document_id: FD-FF7D67A6
title: "Page Specification — Agency Client Report"
...
# Page Specification — Agency Client Report

## Purpose

Defines the complete user experience and implementation contract for the agency client report workflow.

## Scope

This document is the implementation authority for Page Specification — Agency Client Report. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `agency client report` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: agency client report.
- Canonical route: `/app/agency-client-report`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=agency-client-report` and authorized context classification.
- `agency_client_report_primary_action_started`.
...
- Route: `/app/agency-client-report`
