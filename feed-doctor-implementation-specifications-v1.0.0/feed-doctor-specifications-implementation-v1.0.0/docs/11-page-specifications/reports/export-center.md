---
document_id: FD-CEEA8472
title: "Page Specification — Export Center"
...
# Page Specification — Export Center

## Purpose

Defines the complete user experience and implementation contract for the export center workflow.

## Scope

This document is the implementation authority for Page Specification — Export Center. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `export center` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: export center.
- Canonical route: `/app/export-center`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=export-center` and authorized context classification.
- `export_center_primary_action_started`.
