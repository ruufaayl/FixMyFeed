---
document_id: FD-C4E1F79A
title: "Page Specification — Report List"
...
# Page Specification — Report List

## Purpose

Defines the complete user experience and implementation contract for the report list workflow.

## Scope

This document is the implementation authority for Page Specification — Report List. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `report list` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: report list.
- Canonical route: `/app/report-list`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=report-list` and authorized context classification.
- `report_list_primary_action_started`.
