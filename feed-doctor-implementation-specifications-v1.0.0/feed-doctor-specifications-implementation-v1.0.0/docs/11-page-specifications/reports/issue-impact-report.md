---
document_id: FD-5D770229
title: "Page Specification — Issue Impact Report"
...
# Page Specification — Issue Impact Report

## Purpose

Defines the complete user experience and implementation contract for the issue impact report workflow.

## Scope

This document is the implementation authority for Page Specification — Issue Impact Report. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `issue impact report` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: issue impact report.
- Canonical route: `/app/issue-impact-report`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=issue-impact-report` and authorized context classification.
- `issue_impact_report_primary_action_started`.
