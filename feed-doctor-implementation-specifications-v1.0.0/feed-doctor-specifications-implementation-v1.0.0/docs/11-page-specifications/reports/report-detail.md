---
document_id: FD-726C61DA
title: "Page Specification — Report Detail"
...
# Page Specification — Report Detail

## Purpose

Defines the complete user experience and implementation contract for the report detail workflow.

## Scope

This document is the implementation authority for Page Specification — Report Detail. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `report detail` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: report detail.
- Canonical route: `/app/report-detail`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=report-detail` and authorized context classification.
- `report_detail_primary_action_started`.
