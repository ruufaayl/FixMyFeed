---
document_id: FD-129BE853
title: "Page Specification — Executive Report"
...
# Page Specification — Executive Report

## Purpose

Defines the complete user experience and implementation contract for the executive report workflow.

## Scope

This document is the implementation authority for Page Specification — Executive Report. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `executive report` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: executive report.
- Canonical route: `/app/executive-report`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=executive-report` and authorized context classification.
- `executive_report_primary_action_started`.
