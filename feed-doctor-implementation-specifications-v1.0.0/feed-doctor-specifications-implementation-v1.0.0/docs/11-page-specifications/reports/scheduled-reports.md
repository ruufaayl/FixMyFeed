---
document_id: FD-E29B4DA9
title: "Page Specification — Scheduled Reports"
...
# Page Specification — Scheduled Reports

## Purpose

Defines the complete user experience and implementation contract for the scheduled reports workflow.

## Scope

This document is the implementation authority for Page Specification — Scheduled Reports. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `scheduled reports` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: scheduled reports.
- Canonical route: `/app/scheduled-reports`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=scheduled-reports` and authorized context classification.
- `scheduled_reports_primary_action_started`.
