---
document_id: FD-425A38F8
title: "Page Specification — Report Builder"
...
# Page Specification — Report Builder

## Purpose

Defines the complete user experience and implementation contract for the report builder workflow.

## Scope

This document is the implementation authority for Page Specification — Report Builder. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `report builder` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: report builder.
- Canonical route: `/app/report-builder`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=report-builder` and authorized context classification.
- `report_builder_primary_action_started`.
