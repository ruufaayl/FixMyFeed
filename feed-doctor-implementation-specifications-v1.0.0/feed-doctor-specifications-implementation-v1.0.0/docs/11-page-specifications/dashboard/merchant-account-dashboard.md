---
document_id: FD-27F2D858
title: "Page Specification — Merchant Account Dashboard"
...
# Page Specification — Merchant Account Dashboard

## Purpose

Defines the complete user experience and implementation contract for the merchant account dashboard workflow.

## Scope

This document is the implementation authority for Page Specification — Merchant Account Dashboard. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `merchant account dashboard` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: merchant account dashboard.
- Canonical route: `/app/merchant-account-dashboard`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=merchant-account-dashboard` and authorized context classification.
- `merchant_account_dashboard_primary_action_started`.
...
- Route: `/app/merchant-account-dashboard`
