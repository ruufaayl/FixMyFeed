---
document_id: FD-387FB1DC
title: "Page Specification — Connection Inspector"
...
# Page Specification — Connection Inspector

## Purpose

Defines the complete user experience and implementation contract for the connection inspector workflow.

## Scope

This document is the implementation authority for Page Specification — Connection Inspector. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `connection inspector` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: connection inspector.
- Canonical route: `/app/connection-inspector`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=connection-inspector` and authorized context classification.
- `connection_inspector_primary_action_started`.
...
- Route: `/app/connection-inspector`
