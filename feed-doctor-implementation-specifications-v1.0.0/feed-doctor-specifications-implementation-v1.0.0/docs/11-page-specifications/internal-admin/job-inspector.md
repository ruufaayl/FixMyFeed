---
document_id: FD-7AE35FB7
title: "Page Specification — Job Inspector"
...
# Page Specification — Job Inspector

## Purpose

Defines the complete user experience and implementation contract for the job inspector workflow.

## Scope

This document is the implementation authority for Page Specification — Job Inspector. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `job inspector` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: job inspector.
- Canonical route: `/app/job-inspector`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=job-inspector` and authorized context classification.
- `job_inspector_primary_action_started`.
