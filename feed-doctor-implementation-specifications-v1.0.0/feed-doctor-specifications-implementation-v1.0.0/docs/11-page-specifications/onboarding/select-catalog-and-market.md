---
document_id: FD-9BC4B470
title: "Page Specification — Select Catalog And Market"
...
# Page Specification — Select Catalog And Market

## Purpose

Defines the complete user experience and implementation contract for the select catalog and market workflow.

## Scope

This document is the implementation authority for Page Specification — Select Catalog And Market. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `select catalog and market` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: select catalog and market.
- Canonical route: `/app/select-catalog-and-market`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=select-catalog-and-market` and authorized context classification.
- `select_catalog_and_market_primary_action_started`.
...
- Route: `/app/select-catalog-and-market`
