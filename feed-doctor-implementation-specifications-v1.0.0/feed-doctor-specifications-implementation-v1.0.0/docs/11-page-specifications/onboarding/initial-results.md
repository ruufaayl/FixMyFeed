---
document_id: FD-77005AE3
title: "Page Specification — Initial Results"
...
# Page Specification — Initial Results

## Purpose

Defines the complete user experience and implementation contract for the initial results workflow.

## Scope

This document is the implementation authority for Page Specification — Initial Results. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `initial results` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: initial results.
- Canonical route: `/app/initial-results`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=initial-results` and authorized context classification.
- `initial_results_primary_action_started`.
