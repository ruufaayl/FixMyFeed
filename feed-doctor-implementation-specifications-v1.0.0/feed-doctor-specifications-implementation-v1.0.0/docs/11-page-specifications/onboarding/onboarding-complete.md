---
document_id: FD-D45B6C9C
title: "Page Specification — Onboarding Complete"
...
# Page Specification — Onboarding Complete

## Purpose

Defines the complete user experience and implementation contract for the onboarding complete workflow.

## Scope

This document is the implementation authority for Page Specification — Onboarding Complete. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `onboarding complete` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: onboarding complete.
- Canonical route: `/app/onboarding-complete`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=onboarding-complete` and authorized context classification.
- `onboarding_complete_primary_action_started`.
