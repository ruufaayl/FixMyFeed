---
document_id: FD-E6CF816E
title: "Page Specification — Choose Source Platform"
...
# Page Specification — Choose Source Platform

## Purpose

Defines the complete user experience and implementation contract for the choose source platform workflow.

## Scope

This document is the implementation authority for Page Specification — Choose Source Platform. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `choose source platform` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: choose source platform.
- Canonical route: `/app/choose-source-platform`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=choose-source-platform` and authorized context classification.
- `choose_source_platform_primary_action_started`.
...
- Route: `/app/choose-source-platform`
