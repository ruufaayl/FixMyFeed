---
document_id: FD-1E2DA766
title: "Page Specification — Select Merchant Account"
...
# Page Specification — Select Merchant Account

## Purpose

Defines the complete user experience and implementation contract for the select merchant account workflow.

## Scope

This document is the implementation authority for Page Specification — Select Merchant Account. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `select merchant account` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: select merchant account.
- Canonical route: `/app/select-merchant-account`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=select-merchant-account` and authorized context classification.
- `select_merchant_account_primary_action_started`.
...
- Route: `/app/select-merchant-account`
