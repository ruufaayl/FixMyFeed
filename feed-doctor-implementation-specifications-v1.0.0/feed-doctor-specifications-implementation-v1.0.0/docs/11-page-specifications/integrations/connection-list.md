---
document_id: FD-75999999
title: "Page Specification — Connection List"
...
# Page Specification — Connection List

## Purpose

Defines the complete user experience and implementation contract for the connection list workflow.

## Scope

This document is the implementation authority for Page Specification — Connection List. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `connection list` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: connection list.
- Canonical route: `/app/connection-list`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=connection-list` and authorized context classification.
- `connection_list_primary_action_started`.
