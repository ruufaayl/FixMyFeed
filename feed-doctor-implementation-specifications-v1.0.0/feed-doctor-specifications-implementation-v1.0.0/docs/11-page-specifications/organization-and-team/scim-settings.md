---
document_id: FD-C6F5FB3E
title: "Page Specification — Scim Settings"
...
# Page Specification — Scim Settings

## Purpose

Defines the complete user experience and implementation contract for the scim settings workflow.

## Scope

This document is the implementation authority for Page Specification — Scim Settings. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `scim settings` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: scim settings.
- Canonical route: `/app/scim-settings`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=scim-settings` and authorized context classification.
- `scim_settings_primary_action_started`.
