---
document_id: FD-ABAC072D
title: "Page Specification — Service Accounts"
...
# Page Specification — Service Accounts

## Purpose

Defines the complete user experience and implementation contract for the service accounts workflow.

## Scope

This document is the implementation authority for Page Specification — Service Accounts. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `service accounts` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: service accounts.
- Canonical route: `/app/service-accounts`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=service-accounts` and authorized context classification.
- `service_accounts_primary_action_started`.
