---
document_id: FD-9084AAEF
title: "Page Specification — Member Detail"
...
# Page Specification — Member Detail

## Purpose

Defines the complete user experience and implementation contract for the member detail workflow.

## Scope

This document is the implementation authority for Page Specification — Member Detail. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `member detail` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: member detail.
- Canonical route: `/app/member-detail`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=member-detail` and authorized context classification.
- `member_detail_primary_action_started`.
