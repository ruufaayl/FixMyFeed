---
document_id: FD-A3A0A07C
title: "Page Specification — Invite Member"
...
# Page Specification — Invite Member

## Purpose

Defines the complete user experience and implementation contract for the invite member workflow.

## Scope

This document is the implementation authority for Page Specification — Invite Member. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `invite member` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: invite member.
- Canonical route: `/app/invite-member`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=invite-member` and authorized context classification.
- `invite_member_primary_action_started`.
