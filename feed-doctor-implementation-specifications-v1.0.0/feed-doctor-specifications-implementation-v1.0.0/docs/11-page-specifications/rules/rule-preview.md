---
document_id: FD-6417B7F0
title: "Page Specification — Rule Preview"
...
# Page Specification — Rule Preview

## Purpose

Defines the complete user experience and implementation contract for the rule preview workflow.

## Scope

This document is the implementation authority for Page Specification — Rule Preview. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `rule preview` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: rule preview.
- Canonical route: `/app/rule-preview`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=rule-preview` and authorized context classification.
- `rule_preview_primary_action_started`.
