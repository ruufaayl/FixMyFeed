---
document_id: FD-A2FE413D
title: "Page Specification — Rule Test"
...
# Page Specification — Rule Test

## Purpose

Defines the complete user experience and implementation contract for the rule test workflow.

## Scope

This document is the implementation authority for Page Specification — Rule Test. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `rule test` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: rule test.
- Canonical route: `/app/rule-test`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=rule-test` and authorized context classification.
- `rule_test_primary_action_started`.
