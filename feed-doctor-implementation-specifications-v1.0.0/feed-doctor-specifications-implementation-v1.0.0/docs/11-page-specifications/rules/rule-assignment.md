---
document_id: FD-AD1642F7
title: "Page Specification — Rule Assignment"
...
# Page Specification — Rule Assignment

## Purpose

Defines the complete user experience and implementation contract for the rule assignment workflow.

## Scope

This document is the implementation authority for Page Specification — Rule Assignment. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `rule assignment` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: rule assignment.
- Canonical route: `/app/rule-assignment`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=rule-assignment` and authorized context classification.
- `rule_assignment_primary_action_started`.
