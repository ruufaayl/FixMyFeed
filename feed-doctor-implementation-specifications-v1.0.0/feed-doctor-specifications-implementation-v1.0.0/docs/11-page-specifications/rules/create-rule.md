---
document_id: FD-51875250
title: "Page Specification — Create Rule"
...
# Page Specification — Create Rule

## Purpose

Defines the complete user experience and implementation contract for the create rule workflow.

## Scope

This document is the implementation authority for Page Specification — Create Rule. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `create rule` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: create rule.
- Canonical route: `/app/create-rule`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=create-rule` and authorized context classification.
- `create_rule_primary_action_started`.
