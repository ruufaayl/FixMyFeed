---
document_id: FD-88F9F7B3
title: "Page Specification — Rule Builder"
...
# Page Specification — Rule Builder

## Purpose

Defines the complete user experience and implementation contract for the rule builder workflow.

## Scope

This document is the implementation authority for Page Specification — Rule Builder. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `rule builder` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: rule builder.
- Canonical route: `/app/rule-builder`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=rule-builder` and authorized context classification.
- `rule_builder_primary_action_started`.
