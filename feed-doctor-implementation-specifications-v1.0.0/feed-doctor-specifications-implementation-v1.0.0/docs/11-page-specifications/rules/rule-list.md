---
document_id: FD-A293484E
title: "Page Specification — Rule List"
...
# Page Specification — Rule List

## Purpose

Defines the complete user experience and implementation contract for the rule list workflow.

## Scope

This document is the implementation authority for Page Specification — Rule List. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `rule list` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: rule list.
- Canonical route: `/app/rule-list`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=rule-list` and authorized context classification.
- `rule_list_primary_action_started`.
