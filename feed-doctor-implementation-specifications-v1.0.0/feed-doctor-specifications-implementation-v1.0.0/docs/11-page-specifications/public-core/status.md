---
document_id: FD-75C171B5
title: "Page Specification — Status"
...
Defines the complete user experience and implementation contract for the status workflow.

## Scope

This document is the implementation authority for Page Specification — Status. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `status` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: status.
- Canonical route: `/status`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=status` and authorized context classification.
- `status_primary_action_started`.
