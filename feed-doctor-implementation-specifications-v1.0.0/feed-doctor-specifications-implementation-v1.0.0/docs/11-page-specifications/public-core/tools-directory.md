---
document_id: FD-DD6C3CA1
title: "Page Specification — Tools Directory"
...
# Page Specification — Tools Directory

## Purpose

Defines the complete user experience and implementation contract for the tools directory workflow.

## Scope

This document is the implementation authority for Page Specification — Tools Directory. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `tools directory` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: tools directory.
- Canonical route: `/tools-directory`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=tools-directory` and authorized context classification.
- `tools_directory_primary_action_started`.
