---
document_id: FD-F11ED0C1
title: "Page Specification — Attributes Directory"
...
# Page Specification — Attributes Directory

## Purpose

Defines the complete user experience and implementation contract for the attributes directory workflow.

## Scope

This document is the implementation authority for Page Specification — Attributes Directory. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `attributes directory` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: attributes directory.
- Canonical route: `/attributes-directory`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=attributes-directory` and authorized context classification.
- `attributes_directory_primary_action_started`.
