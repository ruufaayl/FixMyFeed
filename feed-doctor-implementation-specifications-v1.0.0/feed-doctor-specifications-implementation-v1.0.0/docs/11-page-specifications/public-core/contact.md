---
document_id: FD-E3814CE0
title: "Page Specification — Contact"
...
Defines the complete user experience and implementation contract for the contact workflow.

## Scope

This document is the implementation authority for Page Specification — Contact. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `contact` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: contact.
- Canonical route: `/contact`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=contact` and authorized context classification.
- `contact_primary_action_started`.
