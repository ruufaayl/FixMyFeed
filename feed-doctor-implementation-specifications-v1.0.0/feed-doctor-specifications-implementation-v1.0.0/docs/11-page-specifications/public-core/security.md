---
document_id: FD-955882BB
title: "Page Specification — Security"
...
# Page Specification — Security

## Purpose

Defines the complete user experience and implementation contract for the security workflow.

## Scope

This document is the implementation authority for Page Specification — Security. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `security` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: security.
- Canonical route: `/security`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=security` and authorized context classification.
- `security_primary_action_started`.
