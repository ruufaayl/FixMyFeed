---
document_id: FD-23C6AE0A
title: "Page Specification — Issue Affected Products"
...
# Page Specification — Issue Affected Products

## Purpose

Defines the complete user experience and implementation contract for the issue affected products workflow.

## Scope

This document is the implementation authority for Page Specification — Issue Affected Products. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `issue affected products` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: issue affected products.
- Canonical route: `/app/issue-affected-products`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=issue-affected-products` and authorized context classification.
- `issue_affected_products_primary_action_started`.
...
- Route: `/app/issue-affected-products`
