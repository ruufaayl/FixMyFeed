---
document_id: FD-1AF9C9F7
title: "Page Specification — Create Scan"
...
# Page Specification — Create Scan

## Purpose

Defines the complete user experience and implementation contract for the create scan workflow.

## Scope

This document is the implementation authority for Page Specification — Create Scan. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `create scan` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: create scan.
- Canonical route: `/app/create-scan`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=create-scan` and authorized context classification.
- `create_scan_primary_action_started`.
