---
document_id: FD-0B0CE3EA
title: "Page Specification — Issue Business Impact"
...
# Page Specification — Issue Business Impact

## Purpose

Defines the complete user experience and implementation contract for the issue business impact workflow.

## Scope

This document is the implementation authority for Page Specification — Issue Business Impact. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `issue business impact` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: issue business impact.
- Canonical route: `/app/issue-business-impact`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=issue-business-impact` and authorized context classification.
- `issue_business_impact_primary_action_started`.
...
- Route: `/app/issue-business-impact`
