---
document_id: FD-C3E44DD8
title: "Page Specification — Diagnostic Baselines"
...
# Page Specification — Diagnostic Baselines

## Purpose

Defines the complete user experience and implementation contract for the diagnostic baselines workflow.

## Scope

This document is the implementation authority for Page Specification — Diagnostic Baselines. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `diagnostic baselines` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: diagnostic baselines.
- Canonical route: `/app/diagnostic-baselines`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=diagnostic-baselines` and authorized context classification.
- `diagnostic_baselines_primary_action_started`.
...
- Route: `/app/diagnostic-baselines`
