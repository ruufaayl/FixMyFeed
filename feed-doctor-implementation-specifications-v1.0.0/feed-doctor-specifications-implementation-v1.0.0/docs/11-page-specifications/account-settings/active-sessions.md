---
document_id: FD-AEAEA180
title: "Page Specification — Active Sessions"
...
# Page Specification — Active Sessions

## Purpose

Defines the complete user experience and implementation contract for the active sessions workflow.

## Scope

This document is the implementation authority for Page Specification — Active Sessions. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `active sessions` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: active sessions.
- Canonical route: `/app/active-sessions`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=active-sessions` and authorized context classification.
- `active_sessions_primary_action_started`.
