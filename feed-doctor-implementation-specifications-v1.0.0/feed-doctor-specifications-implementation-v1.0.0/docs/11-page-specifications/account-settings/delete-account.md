---
document_id: FD-73ED8FE5
title: "Page Specification — Delete Account"
...
# Page Specification — Delete Account

## Purpose

Defines the complete user experience and implementation contract for the delete account workflow.

## Scope

This document is the implementation authority for Page Specification — Delete Account. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `delete account` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: delete account.
- Canonical route: `/app/delete-account`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=delete-account` and authorized context classification.
- `delete_account_primary_action_started`.
