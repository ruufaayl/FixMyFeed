---
document_id: FD-F2FFDE19
title: "Page Specification — Dunning State"
...
# Page Specification — Dunning State

## Purpose

Defines the complete user experience and implementation contract for the dunning state workflow.

## Scope

This document is the implementation authority for Page Specification — Dunning State. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `dunning state` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: dunning state.
- Canonical route: `/app/dunning-state`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=dunning-state` and authorized context classification.
- `dunning_state_primary_action_started`.
