---
document_id: FD-C7F8CA89
title: "Page Specification — Title Checker"
...
# Page Specification — Title Checker

## Purpose

Defines the complete user experience and implementation contract for the title checker workflow.

## Scope

This document is the implementation authority for Page Specification — Title Checker. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `title checker` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: title checker.
- Canonical route: `/title-checker`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=title-checker` and authorized context classification.
- `title_checker_primary_action_started`.
