---
document_id: FD-7A5247CC
title: "Page Specification — Robots Checker"
...
# Page Specification — Robots Checker

## Purpose

Defines the complete user experience and implementation contract for the robots checker workflow.

## Scope

This document is the implementation authority for Page Specification — Robots Checker. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `robots checker` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: robots checker.
- Canonical route: `/robots-checker`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=robots-checker` and authorized context classification.
- `robots_checker_primary_action_started`.
