---
document_id: FD-86E70F8D
title: "Page Specification — Repair Failure"
...
# Page Specification — Repair Failure

## Purpose

Defines the complete user experience and implementation contract for the repair failure workflow.

## Scope

This document is the implementation authority for Page Specification — Repair Failure. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `repair failure` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: repair failure.
- Canonical route: `/app/repair-failure`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=repair-failure` and authorized context classification.
- `repair_failure_primary_action_started`.
