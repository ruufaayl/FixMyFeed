---
document_id: FD-6C2F5D20
title: "Page Specification — Repair Center"
...
# Page Specification — Repair Center

## Purpose

Defines the complete user experience and implementation contract for the repair center workflow.

## Scope

This document is the implementation authority for Page Specification — Repair Center. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `repair center` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: repair center.
- Canonical route: `/app/repair-center`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=repair-center` and authorized context classification.
- `repair_center_primary_action_started`.
