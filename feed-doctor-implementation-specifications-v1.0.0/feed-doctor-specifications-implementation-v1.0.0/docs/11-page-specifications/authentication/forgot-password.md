---
document_id: FD-6814EF40
title: "Page Specification — Forgot Password"
...
# Page Specification — Forgot Password

## Purpose

Defines the complete user experience and implementation contract for the forgot password workflow.

## Scope

This document is the implementation authority for Page Specification — Forgot Password. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `forgot password` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: forgot password.
- Canonical route: `/app/forgot-password`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=forgot-password` and authorized context classification.
- `forgot_password_primary_action_started`.
