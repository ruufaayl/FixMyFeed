---
document_id: FD-0A094910
title: "Page Specification — Reset Password"
...
# Page Specification — Reset Password

## Purpose

Defines the complete user experience and implementation contract for the reset password workflow.

## Scope

This document is the implementation authority for Page Specification — Reset Password. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `reset password` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: reset password.
- Canonical route: `/app/reset-password`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=reset-password` and authorized context classification.
- `reset_password_primary_action_started`.
