---
document_id: FD-EAC0D788
title: "Page Specification — Mfa Challenge"
...
# Page Specification — Mfa Challenge

## Purpose

Defines the complete user experience and implementation contract for the mfa challenge workflow.

## Scope

This document is the implementation authority for Page Specification — Mfa Challenge. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `mfa challenge` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: mfa challenge.
- Canonical route: `/app/mfa-challenge`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=mfa-challenge` and authorized context classification.
- `mfa_challenge_primary_action_started`.
