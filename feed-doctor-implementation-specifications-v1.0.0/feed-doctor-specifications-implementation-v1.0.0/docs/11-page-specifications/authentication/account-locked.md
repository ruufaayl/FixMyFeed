---
document_id: FD-7FAC9EFE
title: "Page Specification — Account Locked"
...
# Page Specification — Account Locked

## Purpose

Defines the complete user experience and implementation contract for the account locked workflow.

## Scope

This document is the implementation authority for Page Specification — Account Locked. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `account locked` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: account locked.
- Canonical route: `/app/account-locked`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=account-locked` and authorized context classification.
- `account_locked_primary_action_started`.
