---
document_id: FD-9B49437A
title: "Page Specification — Email Verification"
...
# Page Specification — Email Verification

## Purpose

Defines the complete user experience and implementation contract for the email verification workflow.

## Scope

This document is the implementation authority for Page Specification — Email Verification. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `email verification` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: email verification.
- Canonical route: `/app/email-verification`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=email-verification` and authorized context classification.
- `email_verification_primary_action_started`.
