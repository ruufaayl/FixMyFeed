---
document_id: FD-B360CCE7
title: "Page Specification — Sign In"
...
# Page Specification — Sign In

## Purpose

Defines the complete user experience and implementation contract for the sign in workflow.

## Scope

This document is the implementation authority for Page Specification — Sign In. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `sign in` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: sign in.
- Canonical route: `/app/sign-in`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=sign-in` and authorized context classification.
- `sign_in_primary_action_started`.
