---
document_id: FD-66B22BB5
title: "Page Specification — Data Source Detail"
...
# Page Specification — Data Source Detail

## Purpose

Defines the complete user experience and implementation contract for the data source detail workflow.

## Scope

This document is the implementation authority for Page Specification — Data Source Detail. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `data source detail` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: data source detail.
- Canonical route: `/app/data-source-detail`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=data-source-detail` and authorized context classification.
- `data_source_detail_primary_action_started`.
