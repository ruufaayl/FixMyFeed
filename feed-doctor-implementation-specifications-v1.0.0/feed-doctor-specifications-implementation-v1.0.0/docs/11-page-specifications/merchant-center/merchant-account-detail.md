---
document_id: FD-FEB872A0
title: "Page Specification — Merchant Account Detail"
...
# Page Specification — Merchant Account Detail

## Purpose

Defines the complete user experience and implementation contract for the merchant account detail workflow.

## Scope

This document is the implementation authority for Page Specification — Merchant Account Detail. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `merchant account detail` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: merchant account detail.
- Canonical route: `/app/merchant-account-detail`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=merchant-account-detail` and authorized context classification.
- `merchant_account_detail_primary_action_started`.
...
- Route: `/app/merchant-account-detail`
