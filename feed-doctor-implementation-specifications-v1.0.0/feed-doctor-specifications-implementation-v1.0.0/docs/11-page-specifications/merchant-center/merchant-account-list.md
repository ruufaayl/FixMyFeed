---
document_id: FD-97EDE2DF
title: "Page Specification — Merchant Account List"
...
# Page Specification — Merchant Account List

## Purpose

Defines the complete user experience and implementation contract for the merchant account list workflow.

## Scope

This document is the implementation authority for Page Specification — Merchant Account List. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `merchant account list` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: merchant account list.
- Canonical route: `/app/merchant-account-list`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=merchant-account-list` and authorized context classification.
- `merchant_account_list_primary_action_started`.
...
- Route: `/app/merchant-account-list`
