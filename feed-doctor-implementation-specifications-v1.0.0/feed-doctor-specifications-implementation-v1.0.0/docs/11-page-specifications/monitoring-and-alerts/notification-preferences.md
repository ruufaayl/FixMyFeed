---
document_id: FD-592AC8F1
title: "Page Specification — Notification Preferences"
...
# Page Specification — Notification Preferences

## Purpose

Defines the complete user experience and implementation contract for the notification preferences workflow.

## Scope

This document is the implementation authority for Page Specification — Notification Preferences. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `notification preferences` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: notification preferences.
- Canonical route: `/app/notification-preferences`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=notification-preferences` and authorized context classification.
- `notification_preferences_primary_action_started`.
