---
document_id: FD-E9475C72
title: "Page Specification — Digest Settings"
...
# Page Specification — Digest Settings

## Purpose

Defines the complete user experience and implementation contract for the digest settings workflow.

## Scope

This document is the implementation authority for Page Specification — Digest Settings. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `digest settings` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: digest settings.
- Canonical route: `/app/digest-settings`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=digest-settings` and authorized context classification.
- `digest_settings_primary_action_started`.
