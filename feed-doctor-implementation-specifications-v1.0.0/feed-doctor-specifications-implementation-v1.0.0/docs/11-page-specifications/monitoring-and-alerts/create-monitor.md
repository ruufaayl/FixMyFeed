---
document_id: FD-83FFED97
title: "Page Specification — Create Monitor"
...
# Page Specification — Create Monitor

## Purpose

Defines the complete user experience and implementation contract for the create monitor workflow.

## Scope

This document is the implementation authority for Page Specification — Create Monitor. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- Completion or safe interruption of the `create monitor` workflow.
- Analytics events, audit events, and navigation outcomes defined below.
- No hidden or ambiguous side effects.

## Functional Requirements

- The page MUST solve one primary workflow: create monitor.
- Canonical route: `/app/create-monitor`; route parameters are opaque IDs or validated slugs.
...
- `page_viewed` with `page_id=create-monitor` and authorized context classification.
- `create_monitor_primary_action_started`.
