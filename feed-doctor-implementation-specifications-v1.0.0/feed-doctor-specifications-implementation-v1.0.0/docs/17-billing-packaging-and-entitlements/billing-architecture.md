---
document_id: FD-C6AC7CFE
title: "Billing and Entitlement Architecture"
...
Defines a provider-neutral billing adapter, local entitlement authority, feature-gated Stripe integration, and safe behavior before billing is enabled.

## Scope

This document is the implementation authority for Billing and Entitlement Architecture. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Billing and Entitlement Architecture.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- `BILLING_ENABLED=false` is the production-safe default and allows owner/admin-assigned internal plans.
- Stripe integration is isolated behind a billing provider interface and added only after owner supplies keys and webhook secret.
- Webhook events are verified, deduplicated, persisted, and reconciled before changing local subscription state.
- Entitlements are derived from versioned plan definitions and durable subscription state.
- Provider outage does not grant unpurchased capabilities, delete data, or block access to already-owned exports.
- Plan limits fail closed with clear upgrade-required states; automatic paid overage is prohibited.
