---
document_id: FD-B034EA7F
title: "Durable Worker Deployment"
...
Defines deployment, scaling, health, shutdown, scheduling, and safety for pg-boss workers.

## Scope

This document is the implementation authority for Durable Worker Deployment. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Durable Worker Deployment.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Worker runs as a long-lived Node.js process with database and storage access but no public application session surface.
- Health endpoints expose liveness and readiness without secrets.
- Graceful shutdown stops claiming jobs, checkpoints active batches, and exits within a configured deadline.
- Concurrency is workload- and tenant-budget aware.
- Only one scheduler leader enqueues recurring work using database coordination.
- Worker deploys use the same immutable release version as web during compatibility windows.
- Worker restarts and duplicate delivery cannot duplicate external side effects.
