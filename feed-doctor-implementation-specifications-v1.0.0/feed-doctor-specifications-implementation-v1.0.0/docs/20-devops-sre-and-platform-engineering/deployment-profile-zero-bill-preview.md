---
document_id: FD-BC6D2EC2
title: "Deployment Profile — Zero-Bill Preview"
...
# Deployment Profile — Zero-Bill Preview

## Purpose

Defines a development and stakeholder-preview deployment that can operate on free tiers but is explicitly not a reliable commercial production environment.

## Scope

This document is the implementation authority for Deployment Profile — Zero-Bill Preview. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Deployment Profile — Zero-Bill Preview.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Host public and application web on a free developer tier only when its terms permit the intended use.
- Use an owner-selected free PostgreSQL tier with strict catalog and retention limits.
- Run workers manually, from a developer machine, or through bounded repository automation for demos only.
- Disable scheduled continuous monitoring, automated writeback, billing, and high-volume public tools.
- Display environment classification in administrative status and prevent accidental production OAuth configuration.
- Do not claim uptime, backup, durability, or scale guarantees.
