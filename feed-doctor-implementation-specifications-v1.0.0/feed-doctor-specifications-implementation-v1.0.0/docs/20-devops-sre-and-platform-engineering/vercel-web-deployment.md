---
document_id: FD-0559BC58
title: "Vercel-Compatible Web Deployment"
...
Defines how the web application is deployed to Vercel without placing durable worker workloads inside request execution.

## Scope

This document is the implementation authority for Vercel-Compatible Web Deployment. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Vercel-Compatible Web Deployment.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Vercel deployment contains the Next.js web application, public pages, authenticated UI, OAuth callbacks, webhook durable-receipt endpoints, and bounded APIs.
- Webhook endpoints verify and persist raw receipt metadata before enqueueing work.
- Long-running imports, scans, remote crawling, report generation, sitemap generation, and writebacks run in the worker deployment.
- Preview deployments use isolated databases and callback credentials or disabled connectors.
- Production environment variables follow the catalog and are scoped by environment.
- Custom domain, canonical host, HTTPS, redirects, and Search Console files are release-gated.
- Deployment must remain reproducible through the Docker path if Vercel pricing or constraints become unsuitable.
