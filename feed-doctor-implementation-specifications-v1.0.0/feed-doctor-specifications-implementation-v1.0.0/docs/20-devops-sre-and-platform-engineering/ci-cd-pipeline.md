...
Defines build, test, security, preview, migration, release, rollback, and evidence stages.

## Scope

This document is the implementation authority for CI/CD Pipeline. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of CI/CD Pipeline.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Pull requests run formatting, lint, type checking, unit, integration, contract, accessibility, SEO, security, and build gates.
- Preview deployments never connect to production databases or OAuth credentials.
- Protected branches require successful gates and review.
- Release artifacts are immutable and traceable to commit, lockfile, migrations, specifications, and test report.
- Production deployment executes preflight, expand migrations, application rollout, smoke tests, and controlled contract migrations.
- Rollback is application-first; database rollback follows the migration plan and may use forward fixes when destructive rollback is unsafe.
