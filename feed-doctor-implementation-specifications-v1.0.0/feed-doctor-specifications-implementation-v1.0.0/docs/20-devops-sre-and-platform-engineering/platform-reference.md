...
Defines the low-cost bootstrap platform, production scale path, environments, CI/CD, observability, backups, and operational ownership.

## Scope

This document is the implementation authority for Platform Reference. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Platform Reference.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Local development uses containers or equivalent reproducible services for PostgreSQL, object storage, SMTP capture, and optional local model.
- CI uses GitHub Actions or an equivalent repository-native runner and must be reproducible locally.
- Web and worker applications build from the same commit and immutable artifact version.
- Production deploys web separately from workers so long-running work is not bound to request runtimes.
- Database migrations run as an explicit gated deployment step, not during arbitrary application startup.
- Secrets are supplied through deployment environment secret stores and never committed.
- Every environment has distinct database, storage, OAuth callbacks, encryption keys, and external app registrations.
- Provider-specific IaC is optional; a provider-neutral deployment manifest and Docker path are mandatory.
...
One web deployment, one worker deployment, one PostgreSQL database, one private object-storage bucket, SMTP, and DNS/CDN. Conservative tenant limits, scheduled scans, and retention keep resource use bounded. Optional services remain disabled.

## Scale Profile

Horizontally scaled stateless web and worker pools; managed PostgreSQL with PITR and replicas; dedicated object storage; stronger edge protection; optional extracted search/analytics/queue only after measured ADR thresholds. Domain behavior and contracts do not change.
