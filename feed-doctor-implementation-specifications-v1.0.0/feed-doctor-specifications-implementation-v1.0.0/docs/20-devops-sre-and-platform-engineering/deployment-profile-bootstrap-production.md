---
document_id: FD-8FCA0D6B
title: "Deployment Profile — Bootstrap Production"
...
# Deployment Profile — Bootstrap Production

## Purpose

Defines the minimum credible low-cost commercial deployment with bounded monthly spend and no mandatory paid API.

## Scope

This document is the implementation authority for Deployment Profile — Bootstrap Production. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Deployment Profile — Bootstrap Production.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Use Vercel-compatible web hosting or the same Docker web image on a small host.
- Use one owner-controlled low-cost Linux instance for durable worker execution and, only when accepted, PostgreSQL; managed PostgreSQL is preferred when budget permits.
- Use private S3-compatible storage or local encrypted storage with off-host backups.
- Enforce product, scan, import, crawl, retention, concurrency, and anonymous-tool limits.
- Maintain database backups off the primary machine and verify restore.
- Use SMTP with daily limits and in-app notifications as authority.
- Set `MONTHLY_EXTERNAL_COST_CAP_USD=0` unless the owner explicitly approves a metered adapter.
- Publish a bootstrap reliability statement rather than scale claims.
...
## Indicative Cost Boundary

The repository does not bind to a vendor price. A small production installation normally incurs recurring compute, database/storage, backups, and email costs even when APIs are free. The owner chooses providers; hard configuration ceilings prevent surprise overage.

## Bootstrap Limits

Default: 5,000 products per tenant, one scheduled diagnostic per day, four worker jobs, three anonymous scans per IP/browser risk identity per day, 24-hour temporary uploads, 7-day exports, and 30-day raw snapshots. Limits are configurable and plan-aware.
