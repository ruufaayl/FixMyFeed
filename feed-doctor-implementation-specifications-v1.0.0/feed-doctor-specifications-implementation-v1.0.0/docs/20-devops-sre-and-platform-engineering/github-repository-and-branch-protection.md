---
document_id: FD-6097AC6C
title: "GitHub Repository and Branch Protection"
...
Defines repository settings, protected branches, reviews, agent permissions, secrets, and release traceability.

## Scope

This document is the implementation authority for GitHub Repository and Branch Protection. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of GitHub Repository and Branch Protection.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Default branch is protected from direct pushes.
- Required checks include all quality gates and migration verification.
- Agent credentials may open branches and pull requests but cannot alter branch protection or production secrets.
- CODEOWNERS requires security/domain review for high-risk directories.
- Secret scanning and dependency alerts are enabled when available.
- Production deployment requires an owner-controlled environment approval.
- Generated artifacts and lockfiles are committed only when the repository policy requires them.
