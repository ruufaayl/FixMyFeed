...
Defines database, object storage, encryption-key, configuration, and restore verification requirements.

## Scope

This document is the implementation authority for Backup and Restore. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Backup and Restore.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Production PostgreSQL has automated encrypted backups and documented retention.
- Bootstrap uses the strongest available low-cost backup method and discloses its RPO.
- Object storage versioning or scheduled copy is enabled for durable artifacts when affordable; temporary objects are excluded.
- Encryption keys are backed up separately from encrypted data using owner-controlled secure storage.
- Restore tests run at least quarterly and before major migrations.
- A backup is not considered valid until a restore test verifies schema, tenant isolation, job state handling, and deletion tombstones.
