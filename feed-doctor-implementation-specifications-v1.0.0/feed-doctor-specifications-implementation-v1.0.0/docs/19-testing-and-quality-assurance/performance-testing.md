...
Defines performance budgets, representative catalogs, load profiles, bottleneck analysis, and capacity gates.

## Scope

This document is the implementation authority for Performance and Scale Testing. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Performance and Scale Testing.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Use synthetic catalogs at 100, 5,000, 100,000, 1,000,000, and larger staged scales.
- Measure web latency, queue age, import throughput, validator throughput, database queries, memory, object-storage bandwidth, and connector call rate.
- Bootstrap limits are tested as hard graceful boundaries.
- Scale claims require measured evidence; architecture readiness is not represented as current capacity.
- Load tests never target third-party production APIs without explicit permission.
