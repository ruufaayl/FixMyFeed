...
Defines automated and manual accessibility verification for WCAG 2.2 AA.

## Scope

This document is the implementation authority for Accessibility Testing. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Accessibility Testing.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Automated accessibility checks run on every critical page and component story.
- Keyboard-only flows cover authentication, onboarding, issue investigation, repair approval, data tables, dialogs, and public tools.
- Screen-reader manual checks cover labels, status announcements, errors, tables, charts, and progress.
- Zoom to 200%, text spacing, high contrast, reduced motion, and mobile target size are verified.
- No release may waive a critical or serious accessibility defect without an approved exception and expiration.
