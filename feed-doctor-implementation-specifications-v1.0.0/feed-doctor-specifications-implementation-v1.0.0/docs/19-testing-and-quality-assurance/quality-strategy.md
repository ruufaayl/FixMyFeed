...
Defines the complete verification model and release gates for agent-produced implementation.

## Scope

This document is the implementation authority for Quality Strategy. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Quality Strategy.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Every requirement ID maps to at least one test ID and implementation artifact.
- Unit tests cover pure domain logic and validators; integration tests cover database, jobs, auth, adapters, and migrations; contract tests cover APIs/events/webhooks; end-to-end tests cover workflows.
- Connector adapters use recorded sanitized fixtures and provider sandbox/test accounts where available.
- High-risk writeback tests use fake connectors by default and owner-controlled test stores before production.
- Security, tenant isolation, accessibility, SEO, migration, backup/restore, and performance are independent release gates.
- Agents cannot delete or weaken tests to achieve green builds without an approved specification change.
- Flaky tests are quarantined only with owner, issue, expiry, and replacement coverage.
...
Formatting; lint; type check; unit; integration; API/event contract; migration up/down safety; dependency and secret scan; tenant-isolation suite; accessibility smoke; SEO route checks; build; end-to-end smoke; generated-spec traceability.

## Coverage Policy

Coverage percentage is not a substitute for behavior coverage. Every state transition, authorization branch, validator reason code, connector error class, repair safety class, and public abuse limit requires named tests.
