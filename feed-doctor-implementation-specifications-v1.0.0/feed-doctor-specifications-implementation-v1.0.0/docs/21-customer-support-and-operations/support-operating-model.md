...
Defines support tiers, access, evidence, escalation, customer communication, and engineering handoff.

## Scope

This document is the implementation authority for Support Operating Model. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Support Operating Model.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Support begins from customer-provided context and diagnostic bundles, not unrestricted database browsing.
- Support access grants are time-boxed, scoped, revocable, customer-visible, and audited.
- Severity 1 covers active security/data-integrity or widespread outage; severity 2 covers major workflow loss; lower severities follow documented response targets.
- Support never promises Google approval, indexing, rankings, or issue resolution timing controlled by external platforms.
- Engineering escalations include reproducible steps, correlation IDs, affected resources, time window, and sanitized evidence.
- Support cannot bypass approval, entitlement, authorization, retention, or security controls.
