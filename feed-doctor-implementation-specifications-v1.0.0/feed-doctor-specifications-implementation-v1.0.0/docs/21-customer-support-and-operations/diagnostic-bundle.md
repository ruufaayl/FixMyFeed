...
Defines a customer-reviewable support export containing enough evidence to troubleshoot without exposing secrets.

## Scope

This document is the implementation authority for Diagnostic Bundle. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Diagnostic Bundle.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Bundle includes application version, connector health, canonical errors, operation timeline, sanitized request metadata, job states, and relevant configuration classifications.
- Bundle excludes access tokens, encryption keys, raw session cookies, payment data, and unrelated catalog content.
- User previews and explicitly grants access before bundle sharing.
- Bundle expires automatically and access is audited.
- Generation works without a third-party support API.
