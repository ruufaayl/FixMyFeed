...
Defines target operators, organizational contexts, permissions, sophistication, catalog scale, and workflow priorities.

## Scope

This document is the implementation authority for User Segmentation. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of User Segmentation.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Primary segments: solo merchant, ecommerce operator, performance marketer, feed specialist, agency manager, enterprise catalog team, developer/integrator, security administrator, billing administrator, support operator.
- Segmentation affects onboarding, defaults, dashboard emphasis, approval policy, reporting, and support—not correctness of diagnostics.
- Catalog size bands: 1–100, 101–5,000, 5,001–100,000, 100,001–1,000,000, and above one million.
- Every persona operates through explicit organization/workspace/store context.
- Agency access is a membership relationship; client data never co-mingles.
