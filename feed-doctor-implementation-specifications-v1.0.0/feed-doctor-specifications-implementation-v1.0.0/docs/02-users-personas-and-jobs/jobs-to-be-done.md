...
Defines the merchant outcomes the product must enable without prescribing implementation.

## Scope

This document is the implementation authority for Jobs to Be Done. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Jobs to Be Done.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Detect feed or account problems before eligibility or revenue is harmed.
- Understand exactly why an issue exists, where the conflicting data originates, and which products and destinations are affected.
- Prioritize work by eligibility, product count, revenue proxy, recurrence, and repair confidence.
- Repair safely at the authoritative source with preview and rollback.
- Verify that the destination processed the change and the issue resolved.
- Prevent regression through continuous monitoring, rules, and alerts.
- Demonstrate feed health and repair outcomes to clients, managers, auditors, or support.
- Find and solve a single issue through a public search page or tool without first learning Feed Doctor.
