---
document_id: FD-4DF4F985
title: "Table Specification — Scheduled Tasks"
...
# Table Specification — Scheduled Tasks

## Purpose

Defines the physical PostgreSQL contract for `scheduled_tasks`, including columns, constraints, indexes, lifecycle, retention, and migration behavior.

## Scope

This document is the implementation authority for Table Specification — Scheduled Tasks. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Table Specification — Scheduled Tasks.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The table MUST store only the `scheduled-tasks` entity or relationship described by its owning domain.
...
The owning domain defines retention. Deletion MUST preserve required audit references, anonymize user-linked fields where mandated, and remove object-storage artifacts through an idempotent deletion job. `scheduled-tasks` data under legal hold cannot be purged.
