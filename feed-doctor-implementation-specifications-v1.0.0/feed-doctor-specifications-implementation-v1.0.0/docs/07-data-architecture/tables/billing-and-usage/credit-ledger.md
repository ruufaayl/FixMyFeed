---
document_id: FD-BC119E96
title: "Table Specification — Credit Ledger"
...
# Table Specification — Credit Ledger

## Purpose

Defines the physical PostgreSQL contract for `credit_ledger`, including columns, constraints, indexes, lifecycle, retention, and migration behavior.

## Scope

This document is the implementation authority for Table Specification — Credit Ledger. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Table Specification — Credit Ledger.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The table MUST store only the `credit-ledger` entity or relationship described by its owning domain.
...
The owning domain defines retention. Deletion MUST preserve required audit references, anonymize user-linked fields where mandated, and remove object-storage artifacts through an idempotent deletion job. `credit-ledger` data under legal hold cannot be purged.
