---
document_id: FD-B564C7B0
title: "Table Specification — Billing Reconciliation Records"
...
# Table Specification — Billing Reconciliation Records

## Purpose

Defines the physical PostgreSQL contract for `billing_reconciliation_records`, including columns, constraints, indexes, lifecycle, retention, and migration behavior.

## Scope

This document is the implementation authority for Table Specification — Billing Reconciliation Records. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Table Specification — Billing Reconciliation Records.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The table MUST store only the `billing-reconciliation-records` entity or relationship described by its owning domain.
...
| `amount` | numeric(20,6) | Yes | Non-floating monetary amount. |
| `currency` | char(3) | Yes | ISO 4217 uppercase code. |
| `quantity` | numeric | Yes | Bounded decimal with explicit unit/context. |
...
The owning domain defines retention. Deletion MUST preserve required audit references, anonymize user-linked fields where mandated, and remove object-storage artifacts through an idempotent deletion job. `billing-reconciliation-records` data under legal hold cannot be purged.
