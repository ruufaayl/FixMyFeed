---
document_id: FD-5FEBD536
title: "Table Specification — Organizations"
...
# Table Specification — Organizations

## Purpose

Defines the physical PostgreSQL contract for `organizations`, including columns, constraints, indexes, lifecycle, retention, and migration behavior.

## Scope

This document is the implementation authority for Table Specification — Organizations. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Table Specification — Organizations.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The table MUST store only the `organizations` entity or relationship described by its owning domain.
...
The owning domain defines retention. Deletion MUST preserve required audit references, anonymize user-linked fields where mandated, and remove object-storage artifacts through an idempotent deletion job. `organizations` data under legal hold cannot be purged.
...
The implementation has unique indexes on `slug` and
`(created_by_user_id, idempotency_key)`, plus the keyset list index
`(created_at DESC, id DESC)`. Organization creation and the initial active
organization-wide administrator membership commit atomically.
