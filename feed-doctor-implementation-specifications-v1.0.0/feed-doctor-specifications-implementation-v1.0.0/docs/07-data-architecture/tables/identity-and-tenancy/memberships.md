---
document_id: FD-92D359FC
title: "Table Specification — Memberships"
...
# Table Specification — Memberships

## Purpose

Defines the physical PostgreSQL contract for `memberships`, including columns, constraints, indexes, lifecycle, retention, and migration behavior.

## Scope

This document is the implementation authority for Table Specification — Memberships. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Table Specification — Memberships.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The table MUST store only the `memberships` entity or relationship described by its owning domain.
...
The owning domain defines retention. Deletion MUST preserve required audit references, anonymize user-linked fields where mandated, and remove object-storage artifacts through an idempotent deletion job. `memberships` data under legal hold cannot be purged.
...
| `workspace_id` | `uuid` | Yes | Composite restrictive foreign key with `organization_id` to the same-tenant workspace. |
| `user_id` | `uuid` | No | Restrictive foreign key to `users.id`. |
| `role` | `text` | No | One canonical RBAC role identifier. |
| `status` | `text` | No | `active`, `suspended`, or `revoked`; defaults to `active`. |
...
| `deleted_at` | `timestamptz` | Yes | Recoverable revoke marker. |

Allowed roles are `viewer`, `operator`, `manager`, `approver`,
`administrator`, `security_administrator`, `billing_administrator`, and
`platform_operator`. Partial unique indexes allow one current organization
membership per user and one current membership per user and workspace.
`(organization_id, created_by_user_id, idempotency_key)` protects retries, and
`(organization_id, created_at DESC, id DESC)` supports keyset lists.
