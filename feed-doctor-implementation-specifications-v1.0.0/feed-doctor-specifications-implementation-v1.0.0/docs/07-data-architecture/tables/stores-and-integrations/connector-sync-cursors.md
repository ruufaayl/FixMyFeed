---
document_id: FD-EB031468
title: "Table Specification — Connector Sync Cursors"
...
# Table Specification — Connector Sync Cursors

## Purpose

Defines the physical PostgreSQL contract for `connector_sync_cursors`, including columns, constraints, indexes, lifecycle, retention, and migration behavior.

## Scope

This document is the implementation authority for Table Specification — Connector Sync Cursors. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Table Specification — Connector Sync Cursors.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The table MUST store only the `connector-sync-cursors` entity or relationship described by its owning domain.
...
| `provider` | text | No | Approved connector identifier. |
| `external_id` | text | Yes | Unique within provider/account scope. |
| `store_id` | uuid | Yes | Same-tenant store foreign key. |
| `connection_id` | uuid | Yes | Encrypted-credential connection foreign key. |
| `market` | text | Yes | Canonical country/market code. |
| `cursor` | text | Yes | Opaque provider pagination cursor. |
| `token_ciphertext` | bytea | Yes | Authenticated encrypted value; never returned. |
| `object_key` | text | Yes | Private object-storage key. |
| `checksum` | text | Yes | Content checksum using approved algorithm. |
| `started_at` | timestamptz | Yes | UTC workflow start. |
| `completed_at` | timestamptz | Yes | UTC terminal completion. |
...
The owning domain defines retention. Deletion MUST preserve required audit references, anonymize user-linked fields where mandated, and remove object-storage artifacts through an idempotent deletion job. `connector-sync-cursors` data under legal hold cannot be purged.
