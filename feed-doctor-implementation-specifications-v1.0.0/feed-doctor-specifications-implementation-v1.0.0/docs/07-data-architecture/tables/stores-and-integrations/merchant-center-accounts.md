---
document_id: FD-0E94628A
title: "Table Specification — Merchant Center Accounts"
...
# Table Specification — Merchant Center Accounts

## Purpose

Defines the physical PostgreSQL contract for `merchant_center_accounts`, including columns, constraints, indexes, lifecycle, retention, and migration behavior.

## Scope

This document is the implementation authority for Table Specification — Merchant Center Accounts. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Table Specification — Merchant Center Accounts.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The table MUST store only the `merchant-center-accounts` entity or relationship described by its owning domain.
...
The owning domain defines retention. Deletion MUST preserve required audit references, anonymize user-linked fields where mandated, and remove object-storage artifacts through an idempotent deletion job. `merchant-center-accounts` data under legal hold cannot be purged.
