---
document_id: FD-36E963BD
title: "Table Specification — Source Schedules"
...
# Table Specification — Source Schedules

## Purpose

Defines the physical PostgreSQL contract for `source_schedules`, including columns, constraints, indexes, lifecycle, retention, and migration behavior.

## Scope

This document is the implementation authority for Table Specification — Source Schedules. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Table Specification — Source Schedules.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The table MUST store only the `source-schedules` entity or relationship described by its owning domain.
...
| `next_run_at` | timestamptz | Yes | Scheduled UTC execution time. |
| `attempt_count` | integer | No | Non-negative attempt count. |
| `config` | jsonb | Yes | Versioned validated configuration. |
...
The owning domain defines retention. Deletion MUST preserve required audit references, anonymize user-linked fields where mandated, and remove object-storage artifacts through an idempotent deletion job. `source-schedules` data under legal hold cannot be purged.
