---
document_id: FD-7A10E84B
title: "Table Specification — Product Destination Mappings"
...
# Table Specification — Product Destination Mappings

## Purpose

Defines the physical PostgreSQL contract for `product_destination_mappings`, including columns, constraints, indexes, lifecycle, retention, and migration behavior.

## Scope

This document is the implementation authority for Table Specification — Product Destination Mappings. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Table Specification — Product Destination Mappings.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The table MUST store only the `product-destination-mappings` entity or relationship described by its owning domain.
...
| `catalog_id` | uuid | Yes | Same-tenant catalog foreign key. |
| `product_id` | uuid | Yes | Same-tenant product foreign key. |
| `variant_id` | uuid | Yes | Same-tenant variant foreign key. |
| `external_id` | text | Yes | Unique within provider/account scope. |
| `market` | text | Yes | Canonical country/market code. |
| `locale` | text | Yes | BCP 47 language tag. |
| `currency` | char(3) | Yes | ISO 4217 uppercase code. |
...
The owning domain defines retention. Deletion MUST preserve required audit references, anonymize user-linked fields where mandated, and remove object-storage artifacts through an idempotent deletion job. `product-destination-mappings` data under legal hold cannot be purged.
