---
document_id: FD-5BBA35A6
title: "Table Specification — Writeback Operations"
...
# Table Specification — Writeback Operations

## Purpose

Defines the physical PostgreSQL contract for `writeback_operations`, including columns, constraints, indexes, lifecycle, retention, and migration behavior.

## Scope

This document is the implementation authority for Table Specification — Writeback Operations. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Table Specification — Writeback Operations.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The table MUST store only the `writeback-operations` entity or relationship described by its owning domain.
...
The owning domain defines retention. Deletion MUST preserve required audit references, anonymize user-linked fields where mandated, and remove object-storage artifacts through an idempotent deletion job. `writeback-operations` data under legal hold cannot be purged.
