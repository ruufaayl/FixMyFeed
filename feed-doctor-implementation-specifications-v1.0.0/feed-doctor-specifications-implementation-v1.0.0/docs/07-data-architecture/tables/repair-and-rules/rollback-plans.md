---
document_id: FD-98BE0D4C
title: "Table Specification — Rollback Plans"
...
# Table Specification — Rollback Plans

## Purpose

Defines the physical PostgreSQL contract for `rollback_plans`, including columns, constraints, indexes, lifecycle, retention, and migration behavior.

## Scope

This document is the implementation authority for Table Specification — Rollback Plans. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Table Specification — Rollback Plans.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The table MUST store only the `rollback-plans` entity or relationship described by its owning domain.
...
The owning domain defines retention. Deletion MUST preserve required audit references, anonymize user-linked fields where mandated, and remove object-storage artifacts through an idempotent deletion job. `rollback-plans` data under legal hold cannot be purged.
