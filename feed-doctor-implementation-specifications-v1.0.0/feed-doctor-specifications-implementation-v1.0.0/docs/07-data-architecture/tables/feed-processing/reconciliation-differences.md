---
document_id: FD-C1D627F4
title: "Table Specification — Reconciliation Differences"
...
# Table Specification — Reconciliation Differences

## Purpose

Defines the physical PostgreSQL contract for `reconciliation_differences`, including columns, constraints, indexes, lifecycle, retention, and migration behavior.

## Scope

This document is the implementation authority for Table Specification — Reconciliation Differences. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Table Specification — Reconciliation Differences.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The table MUST store only the `reconciliation-differences` entity or relationship described by its owning domain.
...
| `source_type` | text | No | Approved source classification. |
...
The owning domain defines retention. Deletion MUST preserve required audit references, anonymize user-linked fields where mandated, and remove object-storage artifacts through an idempotent deletion job. `reconciliation-differences` data under legal hold cannot be purged.
