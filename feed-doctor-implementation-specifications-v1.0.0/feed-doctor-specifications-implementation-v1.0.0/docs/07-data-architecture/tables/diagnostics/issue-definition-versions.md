---
document_id: FD-9749F785
title: "Table Specification — Issue Definition Versions"
...
# Table Specification — Issue Definition Versions

## Purpose

Defines the physical PostgreSQL contract for `issue_definition_versions`, including columns, constraints, indexes, lifecycle, retention, and migration behavior.

## Scope

This document is the implementation authority for Table Specification — Issue Definition Versions. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Table Specification — Issue Definition Versions.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The table MUST store only the `issue-definition-versions` entity or relationship described by its owning domain.
...
| `issue_definition_id` | uuid | Yes | Issue definition/version foreign key. |
| `issue_instance_id` | uuid | Yes | Issue instance foreign key. |
| `severity` | text | Yes | Canonical severity enum. |
| `confidence` | numeric(5,4) | Yes | 0–1 only when mathematically defined. |
| `occurred_at` | timestamptz | No | UTC event occurrence time. |
| `reason` | text | Yes | Bounded safe explanatory reason. |
...
The owning domain defines retention. Deletion MUST preserve required audit references, anonymize user-linked fields where mandated, and remove object-storage artifacts through an idempotent deletion job. `issue-definition-versions` data under legal hold cannot be purged.
