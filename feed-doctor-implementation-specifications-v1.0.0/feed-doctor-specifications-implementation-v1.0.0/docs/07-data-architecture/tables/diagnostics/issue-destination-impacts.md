---
document_id: FD-9E10E381
title: "Table Specification — Issue Destination Impacts"
...
# Table Specification — Issue Destination Impacts

## Purpose

Defines the physical PostgreSQL contract for `issue_destination_impacts`, including columns, constraints, indexes, lifecycle, retention, and migration behavior.

## Scope

This document is the implementation authority for Table Specification — Issue Destination Impacts. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Table Specification — Issue Destination Impacts.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The table MUST store only the `issue-destination-impacts` entity or relationship described by its owning domain.
...
The owning domain defines retention. Deletion MUST preserve required audit references, anonymize user-linked fields where mandated, and remove object-storage artifacts through an idempotent deletion job. `issue-destination-impacts` data under legal hold cannot be purged.
