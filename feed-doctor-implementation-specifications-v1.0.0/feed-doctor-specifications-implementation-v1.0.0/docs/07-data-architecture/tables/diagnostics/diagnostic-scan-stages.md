---
document_id: FD-11F362E7
title: "Table Specification — Diagnostic Scan Stages"
...
# Table Specification — Diagnostic Scan Stages

## Purpose

Defines the physical PostgreSQL contract for `diagnostic_scan_stages`, including columns, constraints, indexes, lifecycle, retention, and migration behavior.

## Scope

This document is the implementation authority for Table Specification — Diagnostic Scan Stages. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Table Specification — Diagnostic Scan Stages.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The table MUST store only the `diagnostic-scan-stages` entity or relationship described by its owning domain.
...
The owning domain defines retention. Deletion MUST preserve required audit references, anonymize user-linked fields where mandated, and remove object-storage artifacts through an idempotent deletion job. `diagnostic-scan-stages` data under legal hold cannot be purged.
