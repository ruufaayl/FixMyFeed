---
document_id: FD-00C8C064
title: "Table Specification — Notification Suppressions"
...
# Table Specification — Notification Suppressions

## Purpose

Defines the physical PostgreSQL contract for `notification_suppressions`, including columns, constraints, indexes, lifecycle, retention, and migration behavior.

## Scope

This document is the implementation authority for Table Specification — Notification Suppressions. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Table Specification — Notification Suppressions.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The table MUST store only the `notification-suppressions` entity or relationship described by its owning domain.
...
The owning domain defines retention. Deletion MUST preserve required audit references, anonymize user-linked fields where mandated, and remove object-storage artifacts through an idempotent deletion job. `notification-suppressions` data under legal hold cannot be purged.
