---
document_id: FD-DAD61D3A
title: "Table Specification — Seo Page Refresh Jobs"
...
# Table Specification — Seo Page Refresh Jobs

## Purpose

Defines the physical PostgreSQL contract for `seo_page_refresh_jobs`, including columns, constraints, indexes, lifecycle, retention, and migration behavior.

## Scope

This document is the implementation authority for Table Specification — Seo Page Refresh Jobs. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Table Specification — Seo Page Refresh Jobs.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The table MUST store only the `seo-page-refresh-jobs` entity or relationship described by its owning domain.
...
| `slug` | text | No | Lowercase canonical slug; unique in scope. |
| `name` | text | No | 1–200 characters; trimmed. |
| `description` | text | Yes | Sanitized plain text. |
| `locale` | text | Yes | BCP 47 language tag. |
| `url` | text | Yes | Normalized validated URL. |
| `checksum` | text | Yes | Content checksum using approved algorithm. |
| `version_number` | integer | No | Positive semantic record version. |
...
The owning domain defines retention. Deletion MUST preserve required audit references, anonymize user-linked fields where mandated, and remove object-storage artifacts through an idempotent deletion job. `seo-page-refresh-jobs` data under legal hold cannot be purged.
