...
Defines authoritative entities, relationships, ownership, temporal behavior, indexing, retention, and transactional boundaries.

## Scope

This document is the implementation authority for Logical Data Model. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Logical Data Model.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Every tenant-owned table MUST include `organization_id`; workspace- or store-scoped data MUST also carry those foreign keys where applicable.
- Primary keys are UUIDv7-compatible identifiers generated application-side.
- All mutable rows use optimistic concurrency through an integer version or immutable operation record.
- Provider identifiers are stored as strings with provider namespace and never treated as globally unique.
- Source snapshots are immutable; normalized current-state rows reference the snapshot that produced them.
- Cross-tenant foreign keys are prohibited and enforced through composite constraints where feasible.
- High-volume occurrence, event, analytics, and audit tables use time-aware partition readiness.
- Deletion uses explicit lifecycle states; soft deletion is not a substitute for retention enforcement.
...
Connection creation, token persistence, and initial sync scheduling are one logical transaction using the outbox. Diagnostic issue upserts and evidence records commit atomically per evaluated batch. Repair approval never shares a transaction with external writeback; the execution state machine bridges the boundary idempotently.

## Indexing Baseline

All tenant-scoped lookup indexes begin with `organization_id`. External callback lookups use hashed state or provider identifiers. Lists use keyset pagination indexes. JSON fields are permitted only for provider payload fragments or versioned extension data and require documented access patterns.
