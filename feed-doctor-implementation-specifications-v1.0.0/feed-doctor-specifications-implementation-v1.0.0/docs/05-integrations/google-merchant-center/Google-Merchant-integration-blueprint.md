---
document_id: FD-A94FDB19
title: "Google Merchant Center Integration Blueprint"
...
# Google Merchant Center Integration Blueprint

## Purpose

Defines OAuth, account discovery, Merchant API versioning, issue ingestion, aggregate statuses, data sources, quotas, resolution actions, and controlled redirection.

## Scope

This document is the implementation authority for Google Merchant Center Integration Blueprint. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Google Merchant Center Integration Blueprint.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Use OAuth authorization code flow with exact redirect URI, state validation, least scopes, encrypted refresh tokens, and revocation handling.
- Discover accessible merchant accounts and account relationships; user selection is mandatory when more than one account is available.
- Pin the Merchant API version behind an adapter and maintain a migration test suite before version changes.
- Ingest account issues, product issues, aggregate product status, product data, destinations, and available issue-resolution content.
- Do not cache Google-provided resolution content beyond permitted behavior; store stable internal issue identity and refresh provider content when displayed.
- Treat triggerable resolution actions as capability-gated because some actions are restricted or allowlisted.
- When an action cannot be executed, produce a deep-link or guided handoff to Merchant Center with preserved context.
- Respect per-method quotas, retry guidance, data processing delay, and country/destination variation.
...
## Issue Identity Strategy

Feed Doctor assigns a stable normalized issue definition and maps Google issue codes, product statuses, severity, destination, country, and API version as aliases. New unknown issues are ingested as `unclassified`, rendered safely, and queued for registry review without blocking the scan.

## Resolution Boundaries

Feed Doctor may safely modify merchant-controlled source data where authorized. It may request provider-side action only when the API exposes and authorizes it. Policy appeals, reviews, account claims, and restricted actions must never be simulated.
