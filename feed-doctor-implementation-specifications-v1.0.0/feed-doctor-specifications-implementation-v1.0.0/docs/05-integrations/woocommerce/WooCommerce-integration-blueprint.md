---
document_id: FD-8DFB4378
title: "WooCommerce Integration Blueprint"
...
Defines credential onboarding, REST API usage, webhooks, reconciliation, writeback, hosting variability, plugin-assisted mode, and disconnect behavior.

## Scope

This document is the implementation authority for WooCommerce Integration Blueprint. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of WooCommerce Integration Blueprint.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Support direct WooCommerce REST API v3 credentials over HTTPS and an optional open-source companion plugin for improved event reliability.
- Validate site URL, TLS, API availability, permission scope, WordPress/WooCommerce versions, and permalink health during connection.
- Use page-based bounded synchronization with checkpointing and adaptive request pacing.
- Webhooks are opportunistic acceleration, not the sole consistency mechanism; periodic reconciliation is mandatory.
- Handle shared-hosting timeouts, disabled cron, caching layers, security plugins, and nonstandard reverse proxies.
- Writeback is disabled by default, uses version checks, and must preserve unknown third-party metadata.
- Disconnect removes stored credentials and leaves an auditable non-secret connection record.
