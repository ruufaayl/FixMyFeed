...
Defines the universal adapter interface for source platforms, destination channels, files, notifications, and billing.

## Scope

This document is the implementation authority for Connector Contract. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Connector Contract.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Every connector publishes capabilities, authentication modes, supported object types, read/write scopes, rate-limit model, webhook support, cursor model, and API-version support.
- Connector methods return normalized results and canonical errors; provider SDK objects cannot escape the adapter boundary.
- Credentials are referenced by encrypted credential ID and never embedded in jobs or events.
- All mutation methods require an idempotency key and return external operation identifiers when available.
- Connectors implement health checks, credential validation, revocation, and data deletion.
- Unsupported capabilities return `CONNECTOR_CAPABILITY_UNAVAILABLE`, never a silent no-op.
...
`catalog.read`, `catalog.write`, `inventory.read`, `inventory.write`, `issues.read`, `issues.resolve`, `accounts.read`, `webhooks.receive`, `bulk.read`, `bulk.write`, `reviews.request`, `billing.manage`, `notifications.send`.

## Canonical Error Classes

Authentication, authorization, revoked, validation, not found, conflict, rate limited, transient upstream, permanent upstream, unsupported capability, quota exhausted, malformed response, and unknown. Unknown errors are non-retryable until classified.
