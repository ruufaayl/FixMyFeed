---
document_id: FD-45652800
title: "Shopify Integration Blueprint"
...
Defines app installation, authentication, GraphQL Admin API usage, bulk synchronization, webhooks, reconciliation, writeback, privacy callbacks, and uninstall behavior.

## Scope

This document is the implementation authority for Shopify Integration Blueprint. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Shopify Integration Blueprint.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Use Shopify OAuth with state and PKCE where supported, exact redirect URIs, least-privilege scopes, and encrypted offline access tokens.
- Use the GraphQL Admin API and pinned supported API version. REST Admin API must not be introduced.
- Initial and large synchronization uses bulk operations; incremental updates use verified webhooks plus periodic reconciliation.
- Required webhook classes include product, inventory, shop update, app uninstall, and mandated privacy/data-erasure callbacks.
- Writeback is disabled by default and limited to approved fields and scopes.
- Every webhook is acknowledged quickly after durable receipt; processing occurs asynchronously.
- Uninstall revokes operation, marks connection unavailable, cancels future syncs, and schedules retention-compliant credential deletion.
...
Shopify product and variant GIDs remain provider identifiers. The normalized model stores product, variant, options, media, category, vendor/brand candidates, prices, inventory, market context, publication state, and selected metafields. The source snapshot remains immutable.

## Conflict Policy

A writeback preview stores the source version and expected Shopify updated timestamp. If either changes before execution, the operation becomes conflicted and requires regeneration; last-write-wins is prohibited.
