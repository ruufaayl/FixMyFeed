---
document_id: FD-F711853E
title: "File and Remote Feed Ingestion Blueprint"
...
# File and Remote Feed Ingestion Blueprint

## Purpose

Defines CSV, TSV, XML, JSON, URL, upload, SFTP, and object-storage ingestion with streaming, schema detection, quarantine, and SSRF defenses.

## Scope

This document is the implementation authority for File and Remote Feed Ingestion Blueprint. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of File and Remote Feed Ingestion Blueprint.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Supported bootstrap formats: CSV, TSV, Google-compatible XML/RSS, and JSON arrays or object collections.
- Files are streamed with compressed and uncompressed size limits, row limits, nesting limits, and time limits.
- Remote fetch validates scheme, DNS resolution, IP ranges, redirects, content type, content length, TLS, and response time.
- Malformed records are quarantined with row or element location while valid records may continue when the parser can recover safely.
- Schema mapping is previewed and versioned before an import becomes active.
- Uploaded temporary files are private, virus-scanned where an open-source scanner is available, and lifecycle-deleted.
