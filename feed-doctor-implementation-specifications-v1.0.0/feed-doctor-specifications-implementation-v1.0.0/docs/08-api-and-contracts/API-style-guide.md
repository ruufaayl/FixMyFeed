...
Defines HTTP resource conventions, authentication, authorization, pagination, filtering, errors, idempotency, concurrency, asynchronous operations, and compatibility.

## Scope

This document is the implementation authority for API Style Guide. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of API Style Guide.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Authenticated product APIs use JSON over HTTPS under `/api/v1` and tenant context derived from authenticated membership, never a client-trusted organization ID alone.
- Resource identifiers are opaque UUIDs. Provider identifiers appear only in provider-specific subresources.
- List endpoints use keyset pagination with `limit` and opaque `cursor`; maximum page size is 100.
- Mutations support `Idempotency-Key` where network retry could duplicate side effects.
- Updates require an `If-Match` entity version or equivalent expected version.
- Long-running requests return `202 Accepted` with an operation resource.
- Errors use the canonical envelope and never return stack traces.
- Breaking changes require a new API version; additive fields are allowed.
...
Single resources return `{ data, meta }`. Collections return `{ data, page: { next_cursor, has_more }, meta }`. Asynchronous requests return `{ operation: { id, status, status_url } }`.

## Canonical Error Envelope

`{ error: { code, message, category, retryable, field_errors, correlation_id, documentation_url } }`. `message` is safe for users; internal causes remain in redacted telemetry.

## HTTP Status Mapping

200 read/update success; 201 create; 202 asynchronous accepted; 204 delete without representation; 400 malformed; 401 unauthenticated; 403 forbidden; 404 absent or intentionally concealed; 409 conflict/idempotency mismatch; 412 stale version; 422 semantic validation; 429 quota or rate limit; 503 transient dependency.
