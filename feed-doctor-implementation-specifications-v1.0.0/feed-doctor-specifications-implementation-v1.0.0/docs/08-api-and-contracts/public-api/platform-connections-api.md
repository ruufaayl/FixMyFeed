---
document_id: FD-A05866F9
title: "Platform Connections API"
...
Defines the versioned HTTP contract for platform connections resources, including routes, representations, authorization, validation, pagination, concurrency, and errors.

## Scope

This document is the implementation authority for Platform Connections API. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Platform Connections API.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Expose only operations required to create, read, update, archive, execute, or inspect platform connections according to its domain lifecycle.
- All routes require authenticated tenant context except explicitly named public-tool routes.
- Responses use the canonical API envelope and stable error codes.
- Collection reads use keyset pagination and bounded filters.
- Mutations require permission checks, audit records, and idempotency where side effects are repeatable.
- Long-running operations return an operation resource and continue through durable jobs.
...
| Method | Path | Purpose |
|---|---|---|
| GET | `/api/v1/platform-connections` | List authorized resources with keyset pagination. |
| POST | `/api/v1/platform-connections` | Create or start the resource operation when supported. |
| GET | `/api/v1/platform-connections/{id}` | Retrieve one resource with authorization and version. |
| PATCH | `/api/v1/platform-connections/{id}` | Update mutable fields using optimistic concurrency. |
| DELETE | `/api/v1/platform-connections/{id}` | Archive, cancel, disconnect, or delete according to lifecycle. |

## Representation Rules

The `platform_connections` representation includes `id`, lifecycle `status`, `version`, timestamps, authorized links, and domain-specific fields. Secrets, encrypted values, internal stack information, and unrelated tenant identifiers are never serialized.

## Authorization Matrix

List: viewer or domain-specific read permission. Create/update: editor or domain-specific manage permission. Delete/execute/high-risk mutation: administrator or explicit specialized permission. Service accounts are scope-limited and cannot use interactive approval bypasses.
