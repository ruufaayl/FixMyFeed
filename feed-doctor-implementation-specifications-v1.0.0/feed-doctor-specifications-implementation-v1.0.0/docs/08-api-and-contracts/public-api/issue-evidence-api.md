---
document_id: FD-1CE935D9
title: "Issue Evidence API"
...
Defines the versioned HTTP contract for issue evidence resources, including routes, representations, authorization, validation, pagination, concurrency, and errors.

## Scope

This document is the implementation authority for Issue Evidence API. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Issue Evidence API.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Expose only operations required to create, read, update, archive, execute, or inspect issue evidence according to its domain lifecycle.
...
| Method | Path | Purpose |
|---|---|---|
| GET | `/api/v1/issue-evidence` | List authorized resources with keyset pagination. |
| POST | `/api/v1/issue-evidence` | Create or start the resource operation when supported. |
| GET | `/api/v1/issue-evidence/{id}` | Retrieve one resource with authorization and version. |
| PATCH | `/api/v1/issue-evidence/{id}` | Update mutable fields using optimistic concurrency. |
| DELETE | `/api/v1/issue-evidence/{id}` | Archive, cancel, disconnect, or delete according to lifecycle. |

## Representation Rules

The `issue_evidence` representation includes `id`, lifecycle `status`, `version`, timestamps, authorized links, and domain-specific fields. Secrets, encrypted values, internal stack information, and unrelated tenant identifiers are never serialized.
