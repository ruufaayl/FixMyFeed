---
document_id: FD-2FC751A3
title: "Public Tools API"
...
Defines the versioned HTTP contract for public tools resources, including routes, representations, authorization, validation, pagination, concurrency, and errors.

## Scope

This document is the implementation authority for Public Tools API. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Public Tools API.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Expose only operations required to create, read, update, archive, execute, or inspect public tools according to its domain lifecycle.
...
| Method | Path | Purpose |
|---|---|---|
| GET | `/api/v1/public-tools` | List authorized resources with keyset pagination. |
| POST | `/api/v1/public-tools` | Create or start the resource operation when supported. |
| GET | `/api/v1/public-tools/{id}` | Retrieve one resource with authorization and version. |
| PATCH | `/api/v1/public-tools/{id}` | Update mutable fields using optimistic concurrency. |
| DELETE | `/api/v1/public-tools/{id}` | Archive, cancel, disconnect, or delete according to lifecycle. |

## Representation Rules

The `public_tools` representation includes `id`, lifecycle `status`, `version`, timestamps, authorized links, and domain-specific fields. Secrets, encrypted values, internal stack information, and unrelated tenant identifiers are never serialized.
