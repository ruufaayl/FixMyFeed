...
Defines the versioned HTTP contract for memberships resources, including routes, representations, authorization, validation, pagination, concurrency, and errors.

## Scope

This document is the implementation authority for Memberships API. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Memberships API.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Expose only operations required to create, read, update, archive, execute, or inspect memberships according to its domain lifecycle.
...
| Method | Path | Purpose |
|---|---|---|
| GET | `/api/v1/memberships` | List authorized resources with keyset pagination. |
| POST | `/api/v1/memberships` | Create or start the resource operation when supported. |
| GET | `/api/v1/memberships/{id}` | Retrieve one resource with authorization and version. |
| PATCH | `/api/v1/memberships/{id}` | Update mutable fields using optimistic concurrency. |
| DELETE | `/api/v1/memberships/{id}` | Archive, cancel, disconnect, or delete according to lifecycle. |

## Representation Rules

The `memberships` representation includes `id`, lifecycle `status`, `version`, timestamps, authorized links, and domain-specific fields. Secrets, encrypted values, internal stack information, and unrelated tenant identifiers are never serialized.
