---
document_id: FD-9BF59310
title: "Merchant Accounts API"
...
Defines the versioned HTTP contract for merchant accounts resources, including routes, representations, authorization, validation, pagination, concurrency, and errors.

## Scope

This document is the implementation authority for Merchant Accounts API. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Merchant Accounts API.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Expose only operations required to create, read, update, archive, execute, or inspect merchant accounts according to its domain lifecycle.
...
| Method | Path | Purpose |
|---|---|---|
| GET | `/api/v1/merchant-accounts` | List authorized resources with keyset pagination. |
| POST | `/api/v1/merchant-accounts` | Create or start the resource operation when supported. |
| GET | `/api/v1/merchant-accounts/{id}` | Retrieve one resource with authorization and version. |
| PATCH | `/api/v1/merchant-accounts/{id}` | Update mutable fields using optimistic concurrency. |
| DELETE | `/api/v1/merchant-accounts/{id}` | Archive, cancel, disconnect, or delete according to lifecycle. |

## Representation Rules

The `merchant_accounts` representation includes `id`, lifecycle `status`, `version`, timestamps, authorized links, and domain-specific fields. Secrets, encrypted values, internal stack information, and unrelated tenant identifiers are never serialized.
