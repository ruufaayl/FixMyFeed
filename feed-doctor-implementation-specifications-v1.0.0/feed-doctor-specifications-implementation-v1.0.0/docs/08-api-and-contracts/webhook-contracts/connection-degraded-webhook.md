---
document_id: FD-3DB31132
title: "Connection Degraded Webhook"
...
# Connection Degraded Webhook

## Purpose

Defines the outbound webhook contract for the connection degraded event.

## Scope

This document is the implementation authority for Connection Degraded Webhook. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Connection Degraded Webhook.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Deliver only to verified HTTPS endpoints.
- Sign the exact raw body with timestamped HMAC and a rotatable endpoint secret.
- Retry transient failures with bounded exponential backoff for no more than the documented retention window.
- Disable an endpoint after repeated terminal failures and notify administrators.
- Provide a unique delivery ID and event ID so receivers can deduplicate.
...
`Feed-Doctor-Event`, `Feed-Doctor-Delivery`, `Feed-Doctor-Timestamp`, `Feed-Doctor-Signature`, and `Content-Type: application/json` are mandatory.

## Response Handling

2xx acknowledges delivery. 408, 425, 429, and 5xx are retryable. Other 4xx responses are terminal except during a short endpoint-recovery grace policy. Redirects are not followed.
