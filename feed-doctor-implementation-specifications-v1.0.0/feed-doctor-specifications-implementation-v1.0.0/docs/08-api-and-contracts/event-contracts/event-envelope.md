...
Defines the immutable internal event schema, delivery semantics, versioning, deduplication, and privacy controls.

## Scope

This document is the implementation authority for Event Envelope. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Event Envelope.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Every event includes event ID, event type, schema version, occurred-at timestamp, producer, organization ID, actor, correlation ID, causation ID, subject type and ID, and minimal payload.
- Events are emitted through a transactional outbox and consumed at least once.
- Consumers must be idempotent using event ID or a domain idempotency key.
- Payloads contain references rather than secrets or large catalog bodies.
- Breaking payload changes create a new schema version with dual-publish migration where required.
