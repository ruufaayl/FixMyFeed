...
Defines all immutable domain events emitted for alert lifecycle changes.

## Scope

This document is the implementation authority for Alert Events. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Alert Events.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Define created, changed, completed, failed, cancelled, and deleted events only where meaningful to the alert lifecycle.
...
|---|---|---|
| `alert.created.v1` | Durable creation committed | resource ID, initial status |
| `alert.updated.v1` | Material authorized change | resource ID, changed field names, version |
| `alert.completed.v1` | Terminal successful workflow | resource ID, outcome summary |
| `alert.failed.v1` | Terminal or operator-actionable failure | resource ID, canonical error code |
| `alert.deleted.v1` | Lifecycle deletion committed | resource ID, deletion mode |
