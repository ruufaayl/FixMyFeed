...
Defines all immutable domain events emitted for privacy lifecycle changes.

## Scope

This document is the implementation authority for Privacy Events. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Privacy Events.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Define created, changed, completed, failed, cancelled, and deleted events only where meaningful to the privacy lifecycle.
...
|---|---|---|
| `privacy.created.v1` | Durable creation committed | resource ID, initial status |
| `privacy.updated.v1` | Material authorized change | resource ID, changed field names, version |
| `privacy.completed.v1` | Terminal successful workflow | resource ID, outcome summary |
| `privacy.failed.v1` | Terminal or operator-actionable failure | resource ID, canonical error code |
| `privacy.deleted.v1` | Lifecycle deletion committed | resource ID, deletion mode |
