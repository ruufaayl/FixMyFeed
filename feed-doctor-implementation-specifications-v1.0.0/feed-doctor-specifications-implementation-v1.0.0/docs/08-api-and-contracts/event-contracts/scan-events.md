...
Defines all immutable domain events emitted for scan lifecycle changes.

## Scope

This document is the implementation authority for Scan Events. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Scan Events.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Define created, changed, completed, failed, cancelled, and deleted events only where meaningful to the scan lifecycle.
...
|---|---|---|
| `scan.created.v1` | Durable creation committed | resource ID, initial status |
| `scan.updated.v1` | Material authorized change | resource ID, changed field names, version |
| `scan.completed.v1` | Terminal successful workflow | resource ID, outcome summary |
| `scan.failed.v1` | Terminal or operator-actionable failure | resource ID, canonical error code |
| `scan.deleted.v1` | Lifecycle deletion committed | resource ID, deletion mode |
