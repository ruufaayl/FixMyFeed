---
document_id: FD-F70B030E
title: "Seo Content Events"
...
Defines all immutable domain events emitted for seo content lifecycle changes.

## Scope

This document is the implementation authority for Seo Content Events. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Seo Content Events.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Define created, changed, completed, failed, cancelled, and deleted events only where meaningful to the seo-content lifecycle.
...
|---|---|---|
| `seo-content.created.v1` | Durable creation committed | resource ID, initial status |
| `seo-content.updated.v1` | Material authorized change | resource ID, changed field names, version |
| `seo-content.completed.v1` | Terminal successful workflow | resource ID, outcome summary |
| `seo-content.failed.v1` | Terminal or operator-actionable failure | resource ID, canonical error code |
| `seo-content.deleted.v1` | Lifecycle deletion committed | resource ID, deletion mode |
