...
Defines all immutable domain events emitted for issue lifecycle changes.

## Scope

This document is the implementation authority for Issue Events. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Issue Events.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Define created, changed, completed, failed, cancelled, and deleted events only where meaningful to the issue lifecycle.
...
|---|---|---|
| `issue.created.v1` | Durable creation committed | resource ID, initial status |
| `issue.updated.v1` | Material authorized change | resource ID, changed field names, version |
| `issue.completed.v1` | Terminal successful workflow | resource ID, outcome summary |
| `issue.failed.v1` | Terminal or operator-actionable failure | resource ID, canonical error code |
| `issue.deleted.v1` | Lifecycle deletion committed | resource ID, deletion mode |
