...
Defines all immutable domain events emitted for store lifecycle changes.

## Scope

This document is the implementation authority for Store Events. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Store Events.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Define created, changed, completed, failed, cancelled, and deleted events only where meaningful to the store lifecycle.
...
|---|---|---|
| `store.created.v1` | Durable creation committed | resource ID, initial status |
| `store.updated.v1` | Material authorized change | resource ID, changed field names, version |
| `store.completed.v1` | Terminal successful workflow | resource ID, outcome summary |
| `store.failed.v1` | Terminal or operator-actionable failure | resource ID, canonical error code |
| `store.deleted.v1` | Lifecycle deletion committed | resource ID, deletion mode |
