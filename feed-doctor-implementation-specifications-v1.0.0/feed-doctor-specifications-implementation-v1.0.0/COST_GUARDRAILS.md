---
document_id: FD-EDBF92BC
title: "Cost Guardrails and Zero-Surprise-Spend Policy"
...
# Cost Guardrails and Zero-Surprise-Spend Policy

## Purpose

This document makes cost containment a release-blocking engineering requirement. “No budget” means no mandatory paid API and no automatic consumption that can create an unapproved bill; it does not claim that real production traffic, storage, bandwidth, backups, email delivery, or compute can remain free indefinitely.

## Scope

This document is the implementation authority for Cost Guardrails and Zero-Surprise-Spend Policy. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Cost Guardrails and Zero-Surprise-Spend Policy.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- No paid LLM, scraping, proxy, enrichment, image-generation, search, analytics, queue, or transactional-email API is required for core functionality.
- Every metered adapter MUST be disabled by default, have an explicit monthly budget, expose usage telemetry, and fail closed when its budget is reached.
- Free-tier limits MUST be configurable rather than embedded in domain logic.
- Public tools MUST enforce anonymous quotas, Turnstile challenges after risk thresholds, input-size limits, concurrency limits, and retention limits.
- Catalog imports MUST enforce per-tenant product, byte, runtime, and scheduling ceilings.
- Workers MUST use fair scheduling so a single tenant cannot consume all compute.
- Storage lifecycle policies MUST delete temporary uploads and generated exports automatically.
- No code path may silently upgrade a provider plan, create billable infrastructure, or continue after a hard cost ceiling.
- Production scale requires owner-approved infrastructure and cannot be honestly promised at zero cost.
