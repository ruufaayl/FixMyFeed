---
document_id: FD-95643134
title: "Agent Bootstrap Instruction"
...
Provides the exact repository-level instruction that starts Claude Code or Codex without authorizing an uncontrolled whole-product build.

## Scope

This document is the implementation authority for Agent Bootstrap Instruction. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Agent Bootstrap Instruction.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Agent must read `AGENTS.md`, `IMPLEMENTATION_BASELINE.md`, `IMPLEMENTATION_SEQUENCE.md`, and `IMPLEMENTATION_STATUS.md`.
- Agent selects only the first Not Started task whose dependencies are Verified.
- Agent opens a dedicated branch, implements the task, runs required gates, updates traceability, and returns the completion report.
- Agent stops after the task and does not automatically continue to the next task.
- Agent may not ask for or use production secrets.
...
Read the repository’s `AGENTS.md` and implementation baseline. Identify the earliest unblocked task in `implementation/WORKSTREAM_REGISTRY.md`. Implement only that task and all its stated acceptance requirements. Use no production credentials and introduce no mandatory paid service. Run all required tests and return the task completion report specified by `AGENTS.md`. Stop after opening the pull request; do not deploy or begin another task.
