---
document_id: FD-37DD6836
title: "Autonomous Engineering Agent Contract"
...
# Autonomous Engineering Agent Contract

## Purpose

This is the controlling instruction document for Claude Code, Codex, and any autonomous implementation agent working in the Feed Doctor repository.

## Scope

This document is the implementation authority for Autonomous Engineering Agent Contract. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Autonomous Engineering Agent Contract.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Read `IMPLEMENTATION_BASELINE.md`, the assigned task, all listed dependencies, and applicable ADRs before modifying files.
- Implement only one approved task scope at a time. Do not infer product behavior that is absent from specifications.
- Never create, request, print, commit, or transmit real credentials.
- Never introduce a mandatory paid service or paid API.
- Never enable billing, automated external writeback, production deletion, or public indexation without the feature gate and owner-supplied configuration.
- Use deterministic business rules. Do not make an LLM a source of truth.
- Create migrations, tests, telemetry, documentation updates, and rollback notes in the same change.
- Run all required quality gates and attach machine-readable evidence.
- Stop and mark Blocked when specifications conflict; do not silently choose.
- Preserve tenant isolation in every query and background job.
...
Every task completion MUST include: task ID; specifications read; files changed; schema changes; API/event changes; security analysis; tests added and results; accessibility evidence; performance evidence; known limitations; rollback procedure; environment variables added; costs introduced; and traceability updates.

## Forbidden Actions

Agents MUST NOT deploy to production, purchase services, change DNS, submit marketplace applications, create irreversible external side effects, weaken tests, bypass authorization, or mark their own high-risk work approved.
