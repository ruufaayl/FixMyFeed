...
Defines Codex-specific operating rules while inheriting `AGENTS.md`.

## Scope

This document is the implementation authority for Codex Repository Instructions. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Codex Repository Instructions.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Codex MUST treat `AGENTS.md` as controlling.
- Use one isolated worktree or branch per task.
- Open reviewable pull requests with exact test evidence and specification references.
- Do not merge or deploy high-risk changes autonomously.
