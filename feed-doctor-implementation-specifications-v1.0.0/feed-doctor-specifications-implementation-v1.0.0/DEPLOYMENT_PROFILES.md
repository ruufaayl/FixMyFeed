...
This document defines a low-cost bootstrap profile and a revenue-backed scale profile that share one implementation, one migration chain, and one behavioral contract.

## Scope

This document is the implementation authority for Deployment Profiles. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Deployment Profiles.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Bootstrap: Vercel-compatible web or a single Docker web service, one PostgreSQL instance, pg-boss workers, local or S3-compatible object storage, SMTP, and first-party analytics.
- Scale: redundant web and worker pools, managed PostgreSQL with read replicas and point-in-time recovery, dedicated object storage, optional dedicated search, regional edge delivery, and audited operational tooling.
- No feature may depend on a scale-only provider; adapters and capacity settings separate profiles.
- Bootstrap MUST enforce conservative quotas and expose upgrade-required states rather than degrade data integrity.
