---
document_id: FD-76F10399
title: "Implementation Status Ledger"
...
This ledger is the machine- and human-readable authority for work state. Agents update it only after evidence is committed and acceptance gates pass.

## Scope

This document is the implementation authority for Implementation Status Ledger. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Implementation Status Ledger.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Allowed states: Not Started, Blocked, In Progress, In Review, Verified, Released, Deprecated.
- Each work item records specification IDs, dependency IDs, owner, branch or pull request, migrations, endpoints, tests, evidence, security review, and release state.
- A work item cannot become Verified with failing or missing required tests.
- Documentation status and code status remain separate; approved specification does not imply implemented behavior.
