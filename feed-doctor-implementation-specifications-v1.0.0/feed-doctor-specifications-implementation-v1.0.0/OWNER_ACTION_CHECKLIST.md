...
Separates owner-controlled credentials, accounts, legal decisions, external approvals, and production release from agent-implementable work.

## Scope

This document is the implementation authority for Owner Action Checklist. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Owner Action Checklist.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Create and control the GitHub repository, protected branch, and deployment environments.
- Supply development/test OAuth app credentials for Shopify and Google only when integration tasks reach that stage.
- Create WooCommerce test store credentials and approve any companion plugin installation.
- Choose and fund bootstrap hosting after reviewing deployment profiles.
- Supply domain and DNS access at launch; keep registrar and production deployment approval owner-controlled.
- Create Stripe account and keys only when billing implementation is reviewed; keep billing disabled before then.
- Review legal documents with qualified counsel before publication.
- Complete Shopify app, Google OAuth verification, or other marketplace/provider approval processes requiring a human account holder.
- Review high-risk writeback tests on controlled test stores.
- Verify Search Console and analytics ownership after agents generate technical artifacts.
- Approve production launch after all gates pass.
