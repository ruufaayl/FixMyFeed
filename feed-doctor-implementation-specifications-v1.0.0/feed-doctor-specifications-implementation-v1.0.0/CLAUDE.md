...
Defines Claude Code-specific operating rules while inheriting `AGENTS.md`.

## Scope

This document is the implementation authority for Claude Code Repository Instructions. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Claude Code Repository Instructions.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Claude Code MUST treat `AGENTS.md` as controlling.
- Use repository tasks in `/implementation/tasks/` and do not process multiple dependency waves in one branch.
- Return a completion report and unresolved conflicts; never substitute assumptions.
