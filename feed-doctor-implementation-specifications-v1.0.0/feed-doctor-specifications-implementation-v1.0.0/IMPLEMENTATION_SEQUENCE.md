...
This document establishes the only approved dependency order for autonomous agents. It prevents agents from building UI or integrations before foundational contracts, tenancy, security, and persistence exist.

## Scope

This document is the implementation authority for Implementation Sequence. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Implementation Sequence.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Wave 0: repository scaffold, CI, linting, test harness, generated contract validation.
- Wave 1: configuration, database migration framework, identity, tenancy, authorization, audit.
- Wave 2: connector framework, encrypted credentials, webhooks, durable jobs, object storage.
- Wave 3: Shopify, WooCommerce, file-source, and Google Merchant adapters.
- Wave 4: catalog normalization, synchronization, reconciliation, snapshots.
- Wave 5: deterministic validators, issue registry, evidence, impact, prioritization.
- Wave 6: repair planning, preview, approval, writeback, verification, rollback.
- Wave 7: application pages, dashboards, rules, monitoring, notifications, reports.
- Wave 8: public tools, SEO content engine, sitemaps, structured data, Search Console readiness.
- Wave 9: billing adapter, entitlement enforcement, administrative operations.
- Wave 10: security, accessibility, load, disaster-recovery, marketplace, and launch gates.
