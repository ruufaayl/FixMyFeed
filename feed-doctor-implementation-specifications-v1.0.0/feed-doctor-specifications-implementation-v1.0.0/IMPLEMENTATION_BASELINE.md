---
document_id: FD-FB948306
title: "Feed Doctor Implementation Baseline"
...
# Feed Doctor Implementation Baseline

## Purpose

This document freezes the production implementation baseline that Claude Code, Codex, and human engineers MUST follow. The baseline eliminates unspecified architectural choices while preserving provider portability and controlled future scaling.

## Scope

This document is the implementation authority for Feed Doctor Implementation Baseline. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Feed Doctor Implementation Baseline.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- The product is a multi-tenant SaaS for Shopify, WooCommerce, file-feed, and Google Merchant Center diagnostics and repair.
- Primary stack: TypeScript monorepo, Next.js 16 App Router, Node.js 24 LTS, PostgreSQL 17+, Drizzle ORM, Better Auth, pg-boss, S3-compatible object storage, SMTP, and PostgreSQL full-text search.
- The diagnostic and repair authority is deterministic. Optional local-model assistance is non-authoritative and disabled by default.
- The web deployment MUST be Vercel-compatible and Docker-portable. Durable workers MUST be deployable separately from request-serving web processes.
- Bootstrap deployment MUST operate without paid AI APIs and MUST enforce hard quotas instead of automatic paid overage.
- Scale deployment MUST preserve the same domain contracts, API contracts, event schemas, migration chain, and source code boundaries.
- Billing, automated writeback, and optional intelligence remain feature-flagged until owner credentials and explicit approval are present.
