---
document_id: FD-BE1AD3FC
title: "Third-Party Dependency Register"
...
This register defines approved libraries, external platforms, licenses, replacement boundaries, cost posture, security ownership, and upgrade policy.

## Scope

This document is the implementation authority for Third-Party Dependency Register. It defines behavior, boundaries, contracts, failure handling, verification, and ownership. Any implementation that conflicts with this document requires an approved ADR and documentation change before merge.
...
- A conforming implementation of Third-Party Dependency Register.
- Traceable tests and operational evidence.
- Auditable state transitions and failure records.

## Functional Requirements

- Core runtime libraries MUST use permissive licenses compatible with commercial SaaS distribution.
- Every dependency requires purpose, package or service owner, pinned major version, license, security advisories, data exposure, cost model, fallback, and removal plan.
- Direct provider SDK use is restricted to adapter modules.
- Unmaintained or license-ambiguous dependencies are prohibited.
- An agent may not add a new service or package without updating this register and obtaining an ADR where architecture is affected.
...
| `better-auth` | Self-hosted authentication framework selected by ADR-006; Feed Doctor Engineering | Exact `1.6.26` | MIT | Review official release notes, npm/GitHub advisories, and `pnpm audit` before upgrades; processes user identity, credential hashes, verification records, and session metadata inside the application boundary | Open source; no recurring fee | Keep policy and request handling behind `@fixmyfeed/auth`; migrate through its versioned core schema and exported application boundary before removal |
| `@better-auth/drizzle-adapter` | Official Better Auth persistence adapter kept behind `@fixmyfeed/database`; Feed Doctor Engineering | Exact `1.6.26`, matching `better-auth` | MIT | Review official release notes, npm/GitHub advisories, and `pnpm audit` before upgrades; maps Better Auth identity and session operations to the approved Drizzle schema | Open source; no recurring fee | Replace inside `@fixmyfeed/database` with another Better Auth-supported PostgreSQL adapter or a reviewed custom adapter without changing application packages |
